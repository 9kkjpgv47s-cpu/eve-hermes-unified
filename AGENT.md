...
- **H5 evidence baseline:** **`npm run bundle:h5-evidence-baseline`** → **`evidence/h5-evidence-baseline-*.json`**; optional **`UNIFIED_RELEASE_READINESS_REQUIRE_H5_BASELINE=1`** in **`release-readiness.mjs`**. Soak SLO validator and **`summarize-evidence.mjs`** both handle **multi-line** soak dispatch JSON and **`soak-*.jsonl`** selection (not **`soak-slo-*.json`**).
