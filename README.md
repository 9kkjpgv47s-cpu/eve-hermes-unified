...
Long-horizon status lives in `docs/HORIZON_STATUS.json` (currently **H1–H20 completed** with **post-H20 sustainment** commands). After `npm install`, periodic operators/agents should:
...
npm run check:goal-policy-coverage -- --source-horizon H2 --until-horizon H20 --require-positive-pending-min --horizon-status-file docs/HORIZON_STATUS.json
npm run audit:goal-policy-readiness -- --source-horizon H2 --max-target-horizon H20 --horizon-status-file docs/HORIZON_STATUS.json
