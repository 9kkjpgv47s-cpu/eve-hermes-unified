...
Long-horizon status lives in `docs/HORIZON_STATUS.json` (currently **H1–H11 completed** with **post-H11 sustainment** commands). After `npm install`, periodic operators/agents should:

```bash
mkdir -p evidence
npm run verify:sustainment-loop
npm run validate:post-h11-sustainment-manifest
```
...
npm run check:goal-policy-coverage -- --source-horizon H2 --until-horizon H6 --require-positive-pending-min --horizon-status-file docs/HORIZON_STATUS.json
