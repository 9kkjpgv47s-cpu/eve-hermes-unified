...
Long-horizon status lives in `docs/HORIZON_STATUS.json` (currently **H1–H19 completed** with **post-H19 sustainment** commands). After `npm install`, periodic operators/agents should:

```bash
mkdir -p evidence
npm run verify:sustainment-loop
npm run validate:post-h19-sustainment-manifest
```
...
npm run check:goal-policy-coverage -- --source-horizon H2 --until-horizon H19 --require-positive-pending-min --horizon-status-file docs/HORIZON_STATUS.json
npm run audit:goal-policy-readiness -- --source-horizon H2 --max-target-horizon H19 --horizon-status-file docs/HORIZON_STATUS.json
