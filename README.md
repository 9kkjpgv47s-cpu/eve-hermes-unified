...
Long-horizon status lives in `docs/HORIZON_STATUS.json` (currently **H1–H6 completed** with **post-H6 sustainment** commands). After `npm install`, periodic operators/agents should:

```bash
mkdir -p evidence
npm run verify:sustainment-loop
npm run validate:post-h6-sustainment-manifest
```
