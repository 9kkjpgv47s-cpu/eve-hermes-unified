...
Long-horizon status lives in `docs/HORIZON_STATUS.json` (currently **H1–H42 completed** with **post-H42 sustainment** commands). After `npm install`, periodic operators/agents should:

```bash
mkdir -p evidence
npm run verify:sustainment-loop
npm run validate:post-h42-sustainment-manifest
```
...
npm run check:goal-policy-coverage -- --source-horizon H2 --until-horizon H42 --require-positive-pending-min --horizon-status-file docs/HORIZON_STATUS.json
