...
Long-horizon status lives in `docs/HORIZON_STATUS.json` (currently **H1–H12 completed** with **post-H12 sustainment** commands). After `npm install`, periodic operators/agents should:

```bash
mkdir -p evidence
npm run verify:sustainment-loop
npm run validate:post-h12-sustainment-manifest
```
