...
- **H16 bundle**: `npm run run:h16-assurance-bundle` chains **`run-h15-assurance-bundle.mjs`** plus **`validate:goal-policy-file`** (through **H22**) and **`validate:manifest-schemas`** (runs before **`validate:all`** in CI).
- **H15 bundle**: **`run-h15-assurance-bundle.mjs`** is **H14-only** (shell resolver smoke); **`validate:shell-unified-dispatch-ci`** runs in **`run-h22-assurance-bundle`** after **`npm run build`**.
- **H17 / H20 / H21**: standalone **`npm run run:h17-assurance-bundle`**, **`run:h20-assurance-bundle`**, **`run:h21-assurance-bundle`** remain for replay; terminal CI uses **`run:h22-assurance-bundle`** which chains **`validate:shell-unified-dispatch-ci`** + **`run-h21-assurance-bundle.mjs`**.
...
- **Closeout gates**: `npm run validate:h22-closeout` (terminal horizon skips downstream stage-promotion in `validate-horizon-closeout`); earlier horizons remain for replay when pinned.
- **Periodic verification**: `npm run verify:sustainment-loop` chains horizon status + **`run:h22-assurance-bundle`** + **`run:ci-soak-slo-gate`** + **`validate:h22-closeout`** → **`evidence/post-h22-sustainment-loop-*.json`**. **`npm run validate:post-h22-sustainment-manifest`** optionally validates the latest manifest. Legacy: **`verify:sustainment-loop:h21-legacy`** / **`validate:post-h21-sustainment-manifest`**; **`verify:sustainment-loop:h20-legacy`** … **`h6-legacy`**.
...
- **CI**: **`unified-ci`** runs **`npm run run:h16-assurance-bundle`** before **`validate:all`**, then **`validate:release-readiness`** / **`validate:initial-scope`**, then **`npm run run:h22-assurance-bundle`** (shell CI scan + H21 chain), then **`verify:sustainment-loop`** / **`validate:post-h22-sustainment-manifest`**, then **`check:stage-promotion-readiness`**.
