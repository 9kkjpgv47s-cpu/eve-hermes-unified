...
- **H16 bundle**: `npm run run:h16-assurance-bundle` chains **`run-h15-assurance-bundle.mjs`** plus **`validate:goal-policy-file`** (through **H30**) and **`validate:manifest-schemas`** (runs before **`validate:all`** in CI).
...
- **Region failover evidence**: `npm run run:region-failover-evidence` runs **`rehearse:region-failover`** and writes **`evidence/region-failover-evidence-*.json`** (machine-readable gate for H5 region rehearsal alignment).
- **Agent remediation evidence**: `npm run run:agent-remediation-evidence` runs **`rehearse:agent-remediation`** and writes **`evidence/agent-remediation-evidence-*.json`** (machine-readable gate for H5 remediation playbook alignment).
- **Emergency rollback evidence**: `npm run run:emergency-rollback-evidence` runs **`rehearse:emergency-rollback`** and writes **`evidence/emergency-rollback-evidence-*.json`** (machine-readable gate for H3 emergency rollback rehearsal alignment).
...
- **Horizon-status gate evidence**: `npm run run:horizon-status-evidence` runs **`validate-horizon-status.mjs`** (same logic as **`npm run validate:horizon-status`**) and writes **`evidence/horizon-status-evidence-*.json`** (machine-readable gate for **`docs/HORIZON_STATUS.json`** schema + metadata alignment after the inner sustainment chain).
...
- **Periodic verification**: `npm run verify:sustainment-loop` runs **`run-post-h30-sustainment-loop.mjs`** (inner **`run-post-h29-sustainment-loop.mjs`** + **`run:horizon-status-evidence`** + **`validate:h30-closeout`**) → **`evidence/post-h30-sustainment-loop-*.json`**. **`npm run validate:post-h30-sustainment-manifest`** optionally validates the latest manifest. Legacy **H29-only** inner chain: **`verify:sustainment-loop:h29-legacy`** / **`validate:post-h29-sustainment-manifest`**. Older: **`verify:sustainment-loop:h28-legacy`** / **`validate:post-h28-sustainment-manifest`**; **`verify:sustainment-loop:h27-legacy`** … **`h6-legacy`**.
