...
- **Contract**: `UNIFIED_DISPATCH_CONTRACT_VERSION` in `src/contracts/schema-version.ts`; every `UnifiedDispatchResult` includes `contractVersion`. Fixtures validate with `validateUnifiedDispatchResult`; run **`npm run validate:dispatch-contract`** (or `tsx src/bin/validate-dispatch-contracts.ts --file …`). Soak SLO helpers: **`npm run validate:soak-matrix`**, **`npm run validate:soak-slo-gate`** (see `scripts/soak-simulate.sh` env toggles).
