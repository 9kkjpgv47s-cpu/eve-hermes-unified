...
- **H5 closeout**: `npm run run:h5-closeout-evidence` writes `evidence/h5-closeout-evidence-*.json`; gate with `npm run validate:h5-closeout`. Stage-promotion readiness is skipped when the next horizon is already **completed** (retroactive closeout) or for terminal **H42** (no downstream horizon).

## Sustainment assurance (terminal H42)

- **Older bundles** (historical): `run:h6-assurance-bundle` … through **`run:h16-assurance-bundle`**.
- **H16 bundle**: `npm run run:h16-assurance-bundle` chains **`run-h15-assurance-bundle.mjs`** plus **`validate:goal-policy-file`** (through **H42**) and **`validate:manifest-schemas`** (runs before **`validate:all`** in CI).
...
- **H23 bundle**: `npm run run:h23-assurance-bundle` runs **`run-post-h22-sustainment-loop.mjs`** then **`validate:manifest-schemas`**; writes **`evidence/h23-assurance-bundle-*.json`**.
...
- **H29 bundle**: `npm run run:h29-assurance-bundle` runs **`run-post-h28-sustainment-loop.mjs`** then **`validate:manifest-schemas`**; writes **`evidence/h29-assurance-bundle-*.json`**.
- **H30 bundle**: `npm run run:h30-assurance-bundle` runs **`run-h29-assurance-bundle.mjs`** then **`validate-horizon-status`** on **`docs/HORIZON_STATUS.json`**; writes **`evidence/h30-assurance-bundle-*.json`**.
- **H31 bundle**: `npm run run:h31-assurance-bundle` runs **`run-post-h30-sustainment-loop.mjs`** then **`validate:manifest-schemas`**; writes **`evidence/h31-assurance-bundle-*.json`**.
- **H32 bundle**: `npm run run:h32-assurance-bundle` runs **`run-h31-assurance-bundle.mjs`** then **`validate-horizon-status`** on **`docs/HORIZON_STATUS.json`**; writes **`evidence/h32-assurance-bundle-*.json`**.
- **H33 bundle**: `npm run run:h33-assurance-bundle` runs **`run-post-h32-sustainment-loop.mjs`** then **`validate:manifest-schemas`**; writes **`evidence/h33-assurance-bundle-*.json`**.
- **H34 bundle**: `npm run run:h34-assurance-bundle` runs **`run-h33-assurance-bundle.mjs`** then **`validate-horizon-status`** on **`docs/HORIZON_STATUS.json`**; writes **`evidence/h34-assurance-bundle-*.json`**.
- **H35 bundle**: `npm run run:h35-assurance-bundle` runs **`run-post-h34-sustainment-loop.mjs`** then **`validate:manifest-schemas`**; writes **`evidence/h35-assurance-bundle-*.json`**.
- **H36 bundle**: `npm run run:h36-assurance-bundle` runs **`run-h35-assurance-bundle.mjs`** then **`validate-horizon-status`** on **`docs/HORIZON_STATUS.json`**; writes **`evidence/h36-assurance-bundle-*.json`**.
- **H37 bundle**: `npm run run:h37-assurance-bundle` runs **`run-post-h36-sustainment-loop.mjs`** then **`validate:manifest-schemas`**; writes **`evidence/h37-assurance-bundle-*.json`**.
- **H38 bundle**: `npm run run:h38-assurance-bundle` runs **`run-h37-assurance-bundle.mjs`** then **`validate-horizon-status`** on **`docs/HORIZON_STATUS.json`**; writes **`evidence/h38-assurance-bundle-*.json`**.
- **H39 bundle**: `npm run run:h39-assurance-bundle` runs **`run-post-h38-sustainment-loop.mjs`** then **`validate:manifest-schemas`**; writes **`evidence/h39-assurance-bundle-*.json`**.
- **H40 bundle**: `npm run run:h40-assurance-bundle` runs **`run-h39-assurance-bundle.mjs`** then **`validate-horizon-status`** on **`docs/HORIZON_STATUS.json`**; writes **`evidence/h40-assurance-bundle-*.json`**.
- **H41 bundle**: `npm run run:h41-assurance-bundle` runs **`run-post-h40-sustainment-loop.mjs`** then **`validate:manifest-schemas`**; writes **`evidence/h41-assurance-bundle-*.json`**.
- **H42 bundle**: `npm run run:h42-assurance-bundle` runs **`run-h41-assurance-bundle.mjs`** then **`validate-horizon-status`** on **`docs/HORIZON_STATUS.json`**; writes **`evidence/h42-assurance-bundle-*.json`**.
- **Closeout gates**: `npm run validate:h42-closeout` (terminal horizon skips downstream stage-promotion in `validate-horizon-closeout`); **`npm run validate:h41-closeout`** … **`validate:h17-closeout`** remain for replay when pinned to earlier horizons.
- **Periodic verification**: `npm run verify:sustainment-loop` chains horizon status + **H17** assurance + **H18** rehearsal + **`run:ci-soak-slo-gate`** + **`run:unified-entrypoints-evidence`** + **`run:shell-unified-dispatch-ci-evidence`** + **`run:tenant-isolation-evidence`** + **`run:h42-assurance-bundle`** + **`validate:h42-closeout`** → **`evidence/post-h42-sustainment-loop-*.json`**. **`npm run validate:post-h42-sustainment-manifest`** optionally validates the latest manifest. Legacy: **`verify:sustainment-loop:h41-legacy`** / **`validate:post-h41-sustainment-manifest`**; **`verify:sustainment-loop:h40-legacy`** / **`validate:post-h40-sustainment-manifest`**; **`verify:sustainment-loop:h39-legacy`** / **`validate:post-h39-sustainment-manifest`**; **`verify:sustainment-loop:h38-legacy`** / **`validate:post-h38-sustainment-manifest`**; **`verify:sustainment-loop:h37-legacy`** / **`validate:post-h37-sustainment-manifest`**; **`verify:sustainment-loop:h36-legacy`** / **`validate:post-h36-sustainment-manifest`**; **`verify:sustainment-loop:h35-legacy`** / **`validate:post-h35-sustainment-manifest`**; **`verify:sustainment-loop:h34-legacy`** / **`validate:post-h34-sustainment-manifest`**; **`verify:sustainment-loop:h33-legacy`** / **`validate:post-h33-sustainment-manifest`**; **`verify:sustainment-loop:h32-legacy`** / **`validate:post-h32-sustainment-manifest`**; **`verify:sustainment-loop:h31-legacy`** / **`validate:post-h31-sustainment-manifest`**; **`verify:sustainment-loop:h30-legacy`** / **`validate:post-h30-sustainment-manifest`**; **`verify:sustainment-loop:h29-legacy`** / **`validate:post-h29-sustainment-manifest`**; **`verify:sustainment-loop:h28-legacy`** / **`validate:post-h28-sustainment-manifest`**; **`verify:sustainment-loop:h27-legacy`** / **`validate:post-h27-sustainment-manifest`**; **`verify:sustainment-loop:h26-legacy`** / **`validate:post-h26-sustainment-manifest`**; **`verify:sustainment-loop:h25-legacy`** / **`validate:post-h25-sustainment-manifest`**; **`verify:sustainment-loop:h24-legacy`** / **`validate:post-h24-sustainment-manifest`**; **`verify:sustainment-loop:h23-legacy`** … **`h6-legacy`**.
