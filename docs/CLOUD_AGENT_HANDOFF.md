...
- **H5 closeout**: `npm run run:h5-closeout-evidence` writes `evidence/h5-closeout-evidence-*.json`; gate with `npm run validate:h5-closeout`. Stage-promotion readiness is skipped when the next horizon is already **completed** (retroactive closeout) or for terminal **H30** (no downstream horizon).

## Sustainment assurance (terminal H30)
...
- **H23 policy bundle** (non-terminal vs **H30**): `npm run run:h23-assurance-bundle` chains **`validate:goal-policy-file`** (through **H30**) + **`validate:manifest-schemas`** + **`run:h22-assurance-bundle`**.
- **H24 sub-bundle** (non-terminal vs **H30**): `npm run run:h24-assurance-bundle` chains **`validate:unified-entrypoints`** + **`validate:tenant-isolation`** + **`rehearse:region-failover`** + **`run:h23-assurance-bundle`** (also invoked inside **`run-h25-assurance-bundle`**).
- **H25 sub-bundle** (non-terminal vs **H30**): `npm run run:h25-assurance-bundle` chains **`run:h6-assurance-bundle`** + **`run:h16-assurance-bundle`** + **`run:h24-assurance-bundle`** (also invoked inside **`run-h26-assurance-bundle`**).
- **H26 sub-bundle** (non-terminal vs **H30**): `npm run run:h26-assurance-bundle` chains **`run-h25-assurance-bundle`** + **`check:stage-promotion-readiness --target-stage canary --allow-horizon-mismatch`** (also invoked inside **`run-h27-assurance-bundle`**).
- **H27 sub-bundle** (non-terminal vs **H30**): `npm run run:h27-assurance-bundle` chains **`validate:horizon-status`** + **`run:h26-assurance-bundle`** (also invoked inside **`run-h28-assurance-bundle`**).
- **H28 sub-bundle** (non-terminal vs **H30**): `npm run run:h28-assurance-bundle` chains **`validate:initial-scope`** + **`run:h27-assurance-bundle`** (also invoked inside **`run-h29-assurance-bundle`**).
- **H29 sub-bundle** (non-terminal vs **H30**): `npm run run:h29-assurance-bundle` chains **`validate:release-readiness`** (same env as **`run-h30-assurance-bundle`**) + **`run:h28-assurance-bundle`** (also invoked inside **`run-h30-assurance-bundle`**).
- **H30 terminal bundle**: `npm run run:h30-assurance-bundle` chains **`validate:all`** + **`run:h29-assurance-bundle`** (folds standalone **`unified-ci`** check/test/build/full-validation into terminal assurance).
...
- **Closeout gates**: `npm run validate:h30-closeout` (terminal horizon skips downstream stage-promotion in `validate-horizon-closeout`); **`npm run validate:h29-closeout`** … **`validate:h17-closeout`** remain for replay when pinned to earlier horizons.
- **Periodic verification**: `npm run verify:sustainment-loop` chains **`run:h30-assurance-bundle`** + **`validate:h30-closeout`** → **`evidence/post-h30-sustainment-loop-*.json`**. **`npm run validate:post-h30-sustainment-manifest`** optionally validates the latest manifest. Legacy H29 chain: **`verify:sustainment-loop:h29-legacy`** / **`validate:post-h29-sustainment-manifest`**; legacy H28 chain: **`verify:sustainment-loop:h28-legacy`** … **`h6-legacy`**.
...
- **CI**: **`unified-ci`** runs **`npm run run:h30-assurance-bundle`** (**`validate:all`** + release readiness + H28 chain), then **`verify:sustainment-loop`** / **`validate:post-h30-sustainment-manifest`**.
