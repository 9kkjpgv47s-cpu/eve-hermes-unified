...
- `npm run audit:goal-policy-readiness -- --source-horizon H2 --until-horizon H10 --horizon-status-file docs/HORIZON_STATUS.json`
...
`npm run validate:all` now includes durability, contract, and scale-slice gates in order: `scan:legacy-dispatch-entrypoints`, `summarize:soak`, `run:emergency-rollback-rehearsal`, `validate:h5-tenant-isolation`, `run:remediation-playbook-dry-run`, `run:h5-region-misalignment-drill`, `run:h6-partition-drill`, `validate:h5-evidence-bundle`, `validate:h6-closeout`, `validate:h6-evidence-bundle`, `validate:h7-closeout`, `validate:h7-evidence-bundle`, `validate:h8-closeout`, `validate:h8-evidence-bundle`, `validate:h9-closeout`, then `validate:regression-eve`, `validate:cutover-readiness`, `emit:validate-all-chain-posture` (writes `validate-all-chain-posture-*.json`, schema `h9-validate-all-chain-v1`, `horizonProgram` H10), `validate:h9-evidence-bundle`, and `validate:h10-closeout`. Full H5 horizon closeout toward H6 uses `npm run validate:h5-closeout` (see `docs/H5_MULTI_TENANT_REGION.md`). H6→H7 promotion pinning uses `npm run validate:h7-closeout` after a passing `validate:h6-evidence-bundle`; H7→H8 uses `npm run validate:h8-closeout` after `validate:h7-evidence-bundle` and requires **`validation-summary` `sloPosture`**; H8→H9 uses `npm run validate:h9-closeout` after `validate:h8-evidence-bundle`; H9→H10 uses `npm run validate:h10-closeout` after `validate:h9-evidence-bundle` and the chain posture (see `docs/H7_PROGRAM.md`, `docs/H8_PROGRAM.md`, `docs/H9_PROGRAM.md`, `docs/H10_PROGRAM.md`).
