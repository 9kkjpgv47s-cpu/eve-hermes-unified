...
5. **H3+ runway** — After H2 closeout is stable, execute planned H3 actions in `docs/HORIZON_STATUS.json` (durability, policy hardening, capability envelopes) with evidence under `evidence/`. **Progress:** all listed **`h3-action-*` items are completed in-repo** (WAL/replay, policy fallback gate, capability timeout, memory durability, soak summarizer, emergency rollback rehearsal manifest); advance to **H4** workstreams next.
