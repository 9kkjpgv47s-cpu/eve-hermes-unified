...
- **Terminal manifest schema evidence (post-H26 loop)**: `npm run run:manifest-schemas-terminal-evidence` runs **`validate:manifest-schemas`** and validates the newest **`post-h26-sustainment-loop-*.json`** against the **`post-h26-sustainment-loop`** schema; writes **`evidence/manifest-schemas-terminal-evidence-*.json`**.
- **Post-H27 loop manifest schema evidence**: `npm run run:manifest-schemas-post-h27-loop-evidence` runs **`validate:manifest-schemas`** and validates the newest **`post-h27-sustainment-loop-*.json`** against the **`post-h27-sustainment-loop`** schema; writes **`evidence/manifest-schemas-post-h27-loop-evidence-*.json`**.
- **Stage-promotion sustainment evidence**: `npm run run:stage-promotion-sustainment-evidence` runs **`check:stage-promotion-readiness`** with **`--target-stage canary --current-stage shadow --allow-horizon-mismatch`**, asserts the newest **`stage-promotion-readiness-*.json`** passes schema + payload, and writes **`evidence/stage-promotion-sustainment-evidence-*.json`**.
...
- **Periodic verification**: `npm run verify:sustainment-loop` runs **`run-post-h28-sustainment-loop.mjs`**, which chains **`run-post-h27-sustainment-loop.mjs`** (horizon status + **H17** + **H18** + CI soak + entrypoints + shell CI + evidence-gates + tenant + region + remediation + emergency rollback + **manifest-schemas-terminal-evidence** + **`validate:h27-closeout`**) + **`run:manifest-schemas-post-h27-loop-evidence`** + **`run:stage-promotion-sustainment-evidence`** + **`validate:h28-closeout`** → **`evidence/post-h28-sustainment-loop-*.json`**. **`npm run validate:post-h28-sustainment-manifest`** optionally validates the latest manifest. Legacy: **`verify:sustainment-loop:h27-legacy`** … **`h22-legacy`**; **`verify:sustainment-loop:h21-legacy`** … **`h6-legacy`**.
