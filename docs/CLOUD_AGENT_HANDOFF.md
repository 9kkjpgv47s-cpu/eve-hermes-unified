...
- **Closeout gates**: **`npm run validate:h30-closeout`** (terminal horizon skips downstream stage-promotion in `validate-horizon-closeout`); **`validate:h22-closeout`** is replayed inside the terminal **`post-h30`** loop for H22 evidence alignment; **`validate:h21-closeout`** … **`validate:h17-closeout`** remain for replay when pinned to earlier horizons.
- **Periodic verification**: **`npm run verify:sustainment-loop`** runs the full **`post-h30`** chain (through **`post-h29`** including manifest pins, dispatch contract fixtures evidence, **`validate:h22-closeout`**, and **`validate:h30-closeout`**) → **`evidence/post-h30-sustainment-loop-*.json`**. **`npm run validate:post-h30-sustainment-manifest`** optionally validates the latest manifest. Legacy: **`verify:sustainment-loop:h29-legacy`** … **`h22-core`**; **`verify:sustainment-loop:h22-legacy-terminal`** replays the old single-manifest H22 terminal chain.
