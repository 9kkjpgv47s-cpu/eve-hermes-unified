...
- **Periodic verification**: `npm run verify:sustainment-loop` runs **`run-post-h23-sustainment-loop.mjs`** (inner **`run-post-h22-sustainment-loop.mjs`** + **`run:region-failover-evidence`** + **`validate:h23-closeout`**) → **`evidence/post-h23-sustainment-loop-*.json`**. **`npm run validate:post-h23-sustainment-manifest`** optionally validates the latest manifest. Legacy **H22-only** inner chain: **`verify:sustainment-loop:h22-legacy`** / **`validate:post-h22-sustainment-manifest`**. Older: **`verify:sustainment-loop:h21-legacy`** / **`validate:post-h21-sustainment-manifest`**; **`verify:sustainment-loop:h20-legacy`** … **`h6-legacy`**.
