...
- **Evidence scripts**: `npm run validate:tenant-isolation`, `npm run rehearse:region-failover`, `npm run rehearse:agent-remediation` (read-only bundle manifest), `npm run rehearse:emergency-rollback` (dry-run manifest by default).
- **H5 closeout**: `npm run run:h5-closeout-evidence` writes `evidence/h5-closeout-evidence-*.json`; gate with `npm run validate:h5-closeout`. Stage-promotion readiness is skipped when the next horizon is already **completed** (retroactive closeout) or for terminal **H27** (no downstream horizon).

## Sustainment assurance (terminal H27)

- **Older bundles** (historical): `run:h6-assurance-bundle` … through **`run:h16-assurance-bundle`**.
- **H16 bundle**: `npm run run:h16-assurance-bundle` chains **`run-h15-assurance-bundle.mjs`** plus **`validate:goal-policy-file`** (through **H27**) and **`validate:manifest-schemas`** (runs before **`validate:all`** in CI).
...
- **Evidence-gates cross-check**: `npm run run:evidence-gates-evidence` pairs **`validation-summary-*.json`** + **`failure-injection-*.txt`** and runs **`evidence-gates.mjs`** → **`evidence/evidence-gates-evidence-*.json`**.
...
- **Region failover evidence**: `npm run run:region-failover-evidence` runs **`rehearse:region-failover`** and writes **`evidence/region-failover-evidence-*.json`**.
- **Agent remediation evidence**: `npm run run:agent-remediation-evidence` runs **`rehearse:agent-remediation`** and writes **`evidence/agent-remediation-evidence-*.json`**.
- **Emergency rollback evidence**: `npm run run:emergency-rollback-evidence` runs **`rehearse:emergency-rollback`** and writes **`evidence/emergency-rollback-evidence-*.json`** (asserts dry-run rehearsal manifest).
- **Terminal manifest schema evidence**: `npm run run:manifest-schemas-terminal-evidence` runs **`validate:manifest-schemas`** and validates the newest **`post-h26-sustainment-loop-*.json`** against the **`post-h26-sustainment-loop`** schema; writes **`evidence/manifest-schemas-terminal-evidence-*.json`**.
- **Closeout gates**: `npm run validate:h27-closeout` (terminal horizon skips downstream stage-promotion in `validate-horizon-closeout`); **`npm run validate:h26-closeout`** … **`validate:h17-closeout`** remain for replay when pinned to earlier horizons.
- **Periodic verification**: `npm run verify:sustainment-loop` chains horizon status + **H17** assurance + **H18** rehearsal + **`run:ci-soak-slo-gate`** + **`run:unified-entrypoints-evidence`** + **`run:shell-unified-dispatch-ci-evidence`** + **`run:evidence-gates-evidence`** + **`run:tenant-isolation-evidence`** + **`run:region-failover-evidence`** + **`run:agent-remediation-evidence`** + **`run:emergency-rollback-evidence`** + **`run:manifest-schemas-terminal-evidence`** + **`validate:h27-closeout`** → **`evidence/post-h27-sustainment-loop-*.json`**. **`npm run validate:post-h27-sustainment-manifest`** optionally validates the latest manifest. Legacy: **`verify:sustainment-loop:h26-legacy`** … **`h22-legacy`**; **`verify:sustainment-loop:h21-legacy`** … **`h6-legacy`**.
