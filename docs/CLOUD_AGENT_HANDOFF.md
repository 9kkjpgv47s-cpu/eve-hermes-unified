...
- `npm run run:stage-drill -- --target-stage <canary|majority|full> --evidence-dir evidence --dry-run`
  - **Dry-run** without **`--current-stage`**: forwards **`--relax-stage-transition`** so readiness does not fail on **shadow→majority** when `gateway.env` still says **shadow** (rehearsal only; production promotions should pass an explicit current stage).
