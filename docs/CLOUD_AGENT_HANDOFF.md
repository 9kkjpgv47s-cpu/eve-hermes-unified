...
- **Periodic verification**: `npm run verify:sustainment-loop` runs **`run-post-h27-sustainment-loop.mjs`** (inner **`run-post-h26-sustainment-loop.mjs`** + **`run:regression-eve-evidence`** + **`validate:h27-closeout`**) → **`evidence/post-h27-sustainment-loop-*.json`**. **`npm run validate:post-h27-sustainment-manifest`** optionally validates the latest manifest. Legacy **H26-only** inner chain: **`verify:sustainment-loop:h26-legacy`** / **`validate:post-h26-sustainment-manifest`**. Older: **`verify:sustainment-loop:h25-legacy`** / **`validate:post-h25-sustainment-manifest`**; **`verify:sustainment-loop:h24-legacy`** … **`h6-legacy`**.
