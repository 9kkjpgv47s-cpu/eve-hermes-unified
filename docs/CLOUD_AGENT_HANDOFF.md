...
- **Closeout gates**: `npm run validate:h22-closeout` (terminal horizon skips downstream stage-promotion in `validate-horizon-closeout`); **`npm run validate:h21-closeout`** … **`validate:h17-closeout`** remain for replay when pinned to earlier horizons.
- **Periodic verification**: `npm run verify:sustainment-loop` runs **`run-post-h22-sustainment-loop.mjs`** (inner **`run-post-h21-sustainment-loop.mjs`** + **`validate:h22-closeout`**) → **`evidence/post-h22-sustainment-loop-*.json`**. **`npm run validate:post-h22-sustainment-manifest`** optionally validates the latest manifest. Legacy **H21-only** inner chain: **`verify:sustainment-loop:h21-legacy`** / **`validate:post-h21-sustainment-manifest`**. Older: **`verify:sustainment-loop:h20-legacy`** / **`validate:post-h20-sustainment-manifest`**; **`verify:sustainment-loop:h19-legacy`** … **`h6-legacy`**.
