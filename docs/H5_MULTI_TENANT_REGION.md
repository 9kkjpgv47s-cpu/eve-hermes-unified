...
After **`validate:h7-closeout`**, **`npm run validate:all`** runs **`validate:h7-evidence-bundle`** then **`validate:h8-closeout`** for the **H7→H8** promotion pin (`h7-closeout-evidence-*.json` → **`h8-closeout-*.json`**, **`--goal-policy-key H7->H8`**). **`validate:h8-closeout`** also requires **`validation-summary-*.json`** to include **`sloPosture`** (`h8-slo-posture-v1`) from **`validate:evidence-summary`**. Then **`validate:h8-evidence-bundle`** and **`validate:h9-closeout`** cover **H8→H9**, followed by **`validate:regression-eve`**, **`validate:cutover-readiness`**, **`npm run emit:validate-all-chain-posture`** (writes **`validate-all-chain-posture-*.json`** with **`horizonProgram: "H10"`**), **`validate:h9-evidence-bundle`**, and **`validate:h10-closeout`**. See **`docs/H7_PROGRAM.md`**, **`docs/H8_PROGRAM.md`**, **`docs/H9_PROGRAM.md`**, and **`docs/H10_PROGRAM.md`**.
