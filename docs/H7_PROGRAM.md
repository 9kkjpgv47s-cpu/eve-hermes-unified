...
5. **H7 slice evidence and H7→H8 (h7-action-3)** — **`npm run validate:h7-evidence-bundle`** writes **`h7-closeout-evidence-*.json`** (`closeout.horizon: "H7"`). **`npm run validate:h8-closeout`** wraps it into **`h8-closeout-*.json`** for **`promote:horizon … --goal-policy-key H7->H8`** and also requires the newest **`validation-summary-*.json`** to carry **`sloPosture`** (`h8-slo-posture-v1`, **`gatesPassed: true`**, **`horizonProgram: "H9"`** when the active program is H9) from **`summarize-evidence`**. **`npm run validate:h7-horizon-closeout`** composes **H7→H8** with **`--require-h7-evidence-bundle`**. **`npm run run:h7-closeout`** passes **`--require-h7-evidence-bundle`**. **`validate:all`** continues with **`validate:h8-evidence-bundle`**, **`validate:h9-closeout`**, **`validate:regression-eve`**, **`validate:cutover-readiness`**, **`emit:validate-all-chain-posture`**, **`validate:h9-evidence-bundle`**, and **`validate:h10-closeout`** (see **`docs/H9_PROGRAM.md`**, **`docs/H10_PROGRAM.md`**).

See **`docs/HORIZON_STATUS.json`** for **`h7-action-*`** status, **`docs/H8_PROGRAM.md`** for the H8 program, **`docs/H9_PROGRAM.md`** for the H9 validate-all slice, **`docs/H10_PROGRAM.md`** for the H10 placeholder, and **`docs/GOAL_POLICIES.json`** for **`H6->H7`**, **`H7->H8`**, **`H8->H9`**, and **`H9->H10`** transition policies.
