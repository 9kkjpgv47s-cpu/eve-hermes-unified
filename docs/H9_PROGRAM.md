...
H9 is the **end-to-end `validate:all` completion** slice: it proves the repository’s unified gate chain finished with **scale evidence**, **SLO posture** on **`validation-summary-*.json`** (`sloPosture.horizonProgram: "H10"` from **`summarize-evidence`** for the active H9→H10 promotion window), **H8→H9 closeout**, **Eve-primary regression**, and **cutover readiness** in one coherent artifact.
...
1. **Pinned evidence chain** — Same ladder as H8, plus an explicit tail:
   ...
   - **`npm run validate:h9-closeout`** → **`h9-closeout-*.json`** (requires newest **`validation-summary-*.json`** with **`sloPosture.gatesPassed: true`**, schema **`h8-slo-posture-v1`**, **`sloPosture.horizonProgram: "H10"`**)
...
4. **H10→H11 continuation** — After **`validate:h10-closeout`**, **`validate:all`** runs **`emit:validate-all-chain-posture-h11`**, **`validate:h10-evidence-bundle`**, and **`validate:h11-closeout`** (see **`docs/H10_PROGRAM.md`**).
...
See **`docs/HORIZON_STATUS.json`** for **`h9-action-*`**, **`docs/H8_PROGRAM.md`** for SLO posture details, **`docs/H10_PROGRAM.md`** for the H10 promotion-pin slice, **`docs/H11_PROGRAM.md`** for the H11 placeholder, and **`docs/GOAL_POLICIES.json`** for **`H8->H9`**, **`H9->H10`**, and **`H10->H11`**.
