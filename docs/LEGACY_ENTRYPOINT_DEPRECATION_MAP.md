...
Run **`npm run validate:dispatch-contract`** (or `npx tsx src/bin/validate-dispatch-contracts.ts`) to validate fixtures; use **`--file <path>`** to validate captured JSON from scripts or CI.
