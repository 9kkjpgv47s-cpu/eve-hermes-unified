...
- [x] `npm run run:h24-assurance-bundle` exits `0` and emits `evidence/h24-assurance-bundle-*.json`.
- [x] `npm run validate:h24-closeout` exits `0` (uses evidence under `evidence/`).

Single command (chains horizon status + H17 assurance + H18 rehearsal + CI soak SLO gate + unified entrypoints evidence + shell CI evidence + tenant isolation evidence + H24 terminal assurance + H24 closeout and writes `evidence/post-h24-sustainment-loop-*.json` with structured `checks.*Pass` booleans):
...
Historical sustainment: **`npm run verify:sustainment-loop:h23-legacy`** / **`validate:post-h23-sustainment-manifest`** (post-H23 chain without H24 terminal slice); **`npm run verify:sustainment-loop:h22-legacy`** / **`validate:post-h22-sustainment-manifest`**; **`npm run verify:sustainment-loop:h21-legacy`** / **`validate:post-h21-sustainment-manifest`**; **`verify:sustainment-loop:h20-legacy`** … **`h6-legacy`**.
