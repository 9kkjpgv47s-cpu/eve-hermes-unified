...
- [ ] `npm run run:h23-assurance-bundle` exits `0` and emits `evidence/h23-assurance-bundle-*.json` (includes H16–H22 sub-bundles + final horizon-status recheck).
- [ ] `npm run validate:evidence-volume` exits `0` and emits `evidence/evidence-volume-report-*.json`.

When **H17**–**H23** closeout is required, run **`npm run validate:h17-closeout`** … **`npm run validate:h23-closeout`** separately (those gates consume the sustainment manifest and must not be nested inside `verify:sustainment-loop`).

Single command (chains horizon status + **terminal** `run:h23-assurance-bundle` + evidence volume; writes `evidence/post-h16-sustainment-loop-*.json` with structured `checks.*Pass` booleans):
