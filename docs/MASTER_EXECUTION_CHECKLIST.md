...
- [ ] `npm run run:h24-assurance-bundle` exits `0` and emits `evidence/h24-assurance-bundle-*.json` (includes H16–H23 sub-bundles + final **`validate:manifest-schemas`** sweep).
- [ ] `npm run validate:evidence-volume` exits `0` and emits `evidence/evidence-volume-report-*.json`.

When **H17**–**H24** closeout is required, run **`npm run validate:h17-closeout`** … **`npm run validate:h24-closeout`** separately (those gates consume the sustainment manifest and must not be nested inside `verify:sustainment-loop`).

Single command (chains horizon status + **terminal** `run:h24-assurance-bundle` + evidence volume; writes `evidence/post-h16-sustainment-loop-*.json` with structured `checks.*Pass` booleans):
