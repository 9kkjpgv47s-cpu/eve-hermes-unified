...
Single command (chains horizon status + H17 assurance + H18 rehearsal + CI soak SLO gate + unified entrypoints evidence + shell CI evidence + H21 closeout and writes `evidence/post-h21-sustainment-loop-*.json` with structured `checks.*Pass` booleans):
...
Historical sustainment: **`npm run verify:sustainment-loop:h20-legacy`** / **`validate:post-h20-sustainment-manifest`** (H20 chain without shell CI artifact); **`npm run verify:sustainment-loop:h19-legacy`** / **`validate:post-h19-sustainment-manifest`**; **`verify:sustainment-loop:h18-legacy`** … **`h6-legacy`**.
