...
- [x] `npm run run:h23-assurance-bundle` exits `0` and emits `evidence/h23-assurance-bundle-*.json` (goal policy + manifest schemas + H22 operational sub-chain).
- [x] `npm run validate:h23-closeout` exits `0` (uses evidence under `evidence/`).

Single command (chains horizon status + H23 assurance bundle + H23 closeout and writes `evidence/post-h23-sustainment-loop-*.json` with structured `checks.*Pass` booleans):
...
Historical sustainment: **`npm run verify:sustainment-loop:h22-legacy`** / **`validate:post-h22-sustainment-manifest`** (legacy post-H22 chain: H17–tenant isolation + H22 closeout); **`npm run verify:sustainment-loop:h21-legacy`** … **`h6-legacy`**.
