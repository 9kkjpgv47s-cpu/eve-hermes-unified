...
- [x] `npm run run:h26-assurance-bundle` exits `0` and emits `evidence/h26-assurance-bundle-*.json`.
- [x] `npm run validate:h26-closeout` exits `0` (uses evidence under `evidence/`).

Single command (chains horizon status + H17 assurance + H18 rehearsal + CI soak SLO gate + unified entrypoints evidence + shell CI evidence + tenant isolation evidence + H26 terminal assurance + H26 closeout and writes `evidence/post-h26-sustainment-loop-*.json` with structured `checks.*Pass` booleans):
