...
Single command (chains horizon status + H17 assurance + H18 rehearsal + CI soak SLO gate + unified entrypoints evidence + shell CI evidence + evidence-gates cross-check + tenant isolation evidence + region failover evidence + agent remediation evidence + H25 closeout and writes `evidence/post-h25-sustainment-loop-*.json` with structured `checks.*Pass` booleans):
...
Historical sustainment: **`npm run verify:sustainment-loop:h24-legacy`** / **`validate:post-h24-sustainment-manifest`** (H24 chain without agent remediation artifact); **`npm run verify:sustainment-loop:h23-legacy`** / **`validate:post-h23-sustainment-manifest`** (H23 chain without region failover artifact); **`npm run verify:sustainment-loop:h22-legacy`** / **`validate:post-h22-sustainment-manifest`**; **`verify:sustainment-loop:h21-legacy`** … **`h6-legacy`**.
