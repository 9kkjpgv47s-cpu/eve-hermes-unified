...
### Horizon H21 - Shell unified-dispatch CI evidence in sustainment (legacy inner chain)
...
### Horizon H22 - Terminal sustainment pin (post-H21 manifest)

Goal: pin the **post-H21** sustainment loop artifact with a dedicated **H22** closeout so operators have a single manifest proving both the inner chain and the pin gate.

Workstreams:

- **`npm run verify:sustainment-loop`**: **`run-post-h22-sustainment-loop.mjs`** runs **`run-post-h21-sustainment-loop.mjs`** then **`validate:h22-closeout`** → **`evidence/post-h22-sustainment-loop-*.json`**.
...
After **H22** is marked completed, use **`npm run verify:sustainment-loop`** (see `docs/MASTER_EXECUTION_CHECKLIST.md` Phase 8). Optionally **`npm run validate:post-h22-sustainment-manifest`**. Legacy **H21-only** inner chain: **`verify:sustainment-loop:h21-legacy`** / **`validate:post-h21-sustainment-manifest`**. Older: **`verify:sustainment-loop:h20-legacy`** / **`validate:post-h20-sustainment-manifest`**, **`verify:sustainment-loop:h19-legacy`** … **`h6-legacy`**.
