...
### Post-H15 operations (sustainment; subsumed by H23 terminal chain)

After **H15** is marked completed, **`npm run run:h15-assurance-bundle`** remains the **H14** shell ingress path (CI scan moved to **H23**); terminal sustainment uses **Post-H23** below. Legacy: **`verify:sustainment-loop:h14-legacy`** … **`h6-legacy`**.

### Horizon H16 - Merge readiness policy gates (goal policy)

Goal: tie **merge-bundle readiness discipline** to executable assurance by chaining **`validate:goal-policy-file`** (full runway **H2→H23**) after the **H15** bundle, so horizon transitions stay aligned with machine-readable policy evidence before **`validate:all`**. The evidence manifest schema sweep runs in **H21** after **`validate:all`**.

Workstreams:

- **`npm run run:h16-assurance-bundle`**: **`run-h15-assurance-bundle.mjs`** + **`validate:goal-policy-file`**; artifact includes **`goalPolicyFileValidationPass`**.
...
- None specific to H16; manifest shape validation is enforced in the **H21** bundle (terminal chain continues through **H23**).
...
Goal: after **`validate:release-readiness`** and **`validate:initial-scope`** produce merge inputs, **single-command** verification replaces standalone **`validate:merge-bundle`** and **`verify:merge-bundle`** steps so CI stays deduplicated and downstream bundles can compose merge readiness without repeating steps. Manifest schema validation runs in **H21**; unified entrypoint validation runs in **H22**; shell CI convergence scan is terminal in **H23**.

Workstreams:

- **`npm run run:h17-assurance-bundle`**: **`validate:merge-bundle`** + **`verify:merge-bundle --latest --no-require-archive`** (reads **`bundle-verification-*.json`** or parses multi-line stdout for **`pass`**).
...
- **`npm run validate:h17-closeout`** passes when evidence is present.
...
### Horizon H18 - Stage-promotion operational assurance

Goal: bundle **merge readiness** (**H17** sub-bundle) with **`check:stage-promotion-readiness`** using the same CI probe (**canary** + **`allow-horizon-mismatch`**) so the terminal sustainment loop exercises the full tail of **`unified-ci`** without a duplicate standalone stage-promotion step.

Workstreams:

- **`npm run run:h18-assurance-bundle`**: invokes **`run-h17-assurance-bundle.mjs`** then **`npm run check:stage-promotion-readiness -- --target-stage canary --allow-horizon-mismatch`**; artifact includes **`h17AssuranceBundlePass`** and **`stagePromotionReadinessPass`**.
- **`npm run validate:h18-closeout`** (legacy sustainment via **`verify:sustainment-loop:h18-legacy`**).

Exit evidence:

- **`npm run run:h18-assurance-bundle`** passes and artifact matches **`evidence/h18-assurance-bundle-*.json`**.
- **`npm run validate:h18-closeout`** passes when evidence is present.

### Horizon H19 - Horizon-status metadata assurance

Goal: fold **`validate:horizon-status`** into the assurance bundle so **`docs/HORIZON_STATUS.json`** schema and goal-policy duplicate-key checks run in the same artifact chain as merge/stage gates (CI no longer runs **`validate:horizon-status`** as a separate step after sustainment).
...
- **`validate:horizon-status`** stdout must parse as JSON with **`valid: true`**; the **H19** bundle parses embedded JSON like **`verify:merge-bundle`**.
...
Goal: enforce **`validate:evidence-gates`** (cross-check **`validation-summary-*.json`** against **`failure-injection-*.txt`** with required failure scenarios + P95 bound) inside the assurance chain after **`validate:all`** produces both artifacts.
...
- **`npm run validate:h20-closeout`** (legacy sustainment via **`verify:sustainment-loop:h20-legacy`**).

Exit evidence:

- **`npm run run:h20-assurance-bundle`** passes and artifact matches **`evidence/h20-assurance-bundle-*.json`**.
- **`npm run validate:h20-closeout`** passes when evidence is present (**H20** is not terminal once **H23** exists; stage-promotion readiness skipped only when validating terminal **H23**).
...
- Run after **`npm run validate:all`** (CI order: unit tests → **`validate:all`** → release readiness → initial scope → **H23** bundle).

### Horizon H21 - Manifest schema assurance

Goal: enforce **`validate:manifest-schemas`** on **`evidence/`** after **`validate:all`** populates manifests, chaining **`run-h20-assurance-bundle.mjs`** so schema consistency rides the assurance tail without duplicate CI steps.

Workstreams:

- **`npm run run:h21-assurance-bundle`**: **`validate:manifest-schemas`** then **`run-h20-assurance-bundle.mjs`**; artifact includes **`manifestSchemasPass`** and **`h20AssuranceBundlePass`**.
- **`npm run validate:h21-closeout`** (legacy sustainment via **`verify:sustainment-loop:h21-legacy`**).

Exit evidence:

- **`npm run run:h21-assurance-bundle`** passes and artifact matches **`evidence/h21-assurance-bundle-*.json`**.
- **`npm run validate:h21-closeout`** passes when evidence is present (**H21** is not terminal once **H23** exists).

Primary risks:

- **`validate:manifest-schemas`** requires JSON under **`evidence/`**; running before **`validate:all`** yields thin coverage.

Mitigations:

- CI runs **H21** as part of **`run:h23-assurance-bundle`** after **`validate:all`** + release readiness + initial scope.

### Horizon H22 - Unified entrypoints assurance (sub-chain)

Goal: enforce **`validate:unified-entrypoints`** after **`validate:all`** (which runs **`npm run build`**) so adapter instantiation stays confined to **`unified-dispatch`**, chaining **`run-h21-assurance-bundle.mjs`** without duplicate early CI steps. **H6–H12** sustainment bundles no longer rerun this scan (post-build gate lives here).

Workstreams:

- **`npm run run:h22-assurance-bundle`**: **`validate:unified-entrypoints`** then **`run-h21-assurance-bundle.mjs`**; artifact includes **`unifiedEntrypointsPass`** and **`h21AssuranceBundlePass`**.
- **`npm run validate:h22-closeout`** (legacy sustainment via **`verify:sustainment-loop:h22-legacy`**).

Exit evidence:

- **`npm run run:h22-assurance-bundle`** passes and artifact matches **`evidence/h22-assurance-bundle-*.json`**.
- **`npm run validate:h22-closeout`** passes when evidence is present (**H22** is not terminal once **H23** exists).

Primary risks:

- **`validate:unified-entrypoints`** scans **`src/**/*.ts`**; it must run after **`build`** produces consistent sources ( **`validate:all`** covers **`check`**, **`test`**, **`build`**).

Mitigations:

- CI runs **H22** as the sub-chain inside **`run:h23-assurance-bundle`** after **`validate:all`** + release readiness + initial scope.

### Horizon H23 - Shell CI convergence terminal assurance

Goal: run **`validate:shell-unified-dispatch-ci`** after **`npm run build`** so repo shell scripts align with **`dist/`** unified-dispatch paths, chaining **`run-h22-assurance-bundle.mjs`** as the terminal assurance step ( **`validate:shell-unified-dispatch-ci`** moved out of **`run-h15-assurance-bundle`**).

Workstreams:

- **`npm run run:h23-assurance-bundle`**: **`validate:shell-unified-dispatch-ci`** then **`run-h22-assurance-bundle.mjs`**; artifact includes **`shellUnifiedDispatchCiScanPass`** and **`h22AssuranceBundlePass`**.
- **`npm run validate:h23-closeout`**, **`npm run verify:sustainment-loop`** (post-H23), **`npm run validate:post-h23-sustainment-manifest`**.

Exit evidence:

- **`npm run run:h23-assurance-bundle`** passes and artifact matches **`evidence/h23-assurance-bundle-*.json`**.
...
- Shell scripts may legitimately reference **`dist/`** paths after build; running the scan before **`build`** causes false failures.

Mitigations:

- CI runs **H23** after **`validate:all`** + release readiness + initial scope.
