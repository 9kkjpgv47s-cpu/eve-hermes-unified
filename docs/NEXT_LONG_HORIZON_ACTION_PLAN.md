...
### Horizon H28 - Terminal sustainment pin (post-H27 manifest + evidence-summary gate evidence)

Goal: pin the **post-H27** sustainment loop artifact with a dedicated **H28** closeout and add **machine-readable** **`validate:evidence-summary`** evidence so validation-summary aggregation stays on the same **`evidence/`** snapshot as the inner sustainment chain.
...
- **`npm run verify:sustainment-loop`**: **`run-post-h28-sustainment-loop.mjs`** runs **`run-post-h27-sustainment-loop.mjs`** then **`run:evidence-summary-evidence`** then **`validate:h28-closeout`** → **`evidence/post-h28-sustainment-loop-*.json`**.
