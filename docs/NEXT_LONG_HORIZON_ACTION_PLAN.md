...
- **`npm run validate:h11-closeout`** passes when evidence is present (stage-promotion readiness skipped when the next horizon is already completed or when validating terminal **H25**).
...
- **`npm run validate:h12-closeout`** passes when evidence is present (stage-promotion readiness skipped when the next horizon is already completed or when validating terminal **H25**).
...
- **`npm run validate:h13-closeout`** passes when evidence is present (stage-promotion readiness skipped when the next horizon is already completed or when validating terminal **H25**).
...
After **H15** is marked completed, **`npm run run:h15-assurance-bundle`** remains the shell CI convergence path; terminal sustainment uses **Post-H25** below. Legacy: **`verify:sustainment-loop:h14-legacy`** … **`h6-legacy`**.

### Horizon H16 - Merge readiness policy gates (goal policy + manifest schemas)

Goal: tie **merge-bundle readiness discipline** to executable assurance by chaining **`validate:goal-policy-file`** (full runway **H2→H25**) and **`validate:manifest-schemas`** on **`evidence/`** after the **H15** bundle, so horizon transitions stay aligned with machine-readable evidence shapes before **`validate:all`**.
...
- **`npm run validate:h16-closeout`** passes when evidence is present (retroactive closeout when **H25** is terminal).
...
- **`npm run validate:h17-closeout`** passes when evidence is present (retroactive closeout when **H25** is terminal).
...
- **`npm run validate:h18-closeout`** (H18 replay when **H25** is terminal).

Exit evidence:

- **`npm run run:h18-assurance-bundle`** passes and artifact matches **`evidence/h18-assurance-bundle-*.json`**.
- **`npm run validate:h18-closeout`** passes when evidence is present (retroactive closeout when **H25** is terminal).
...
- **`npm run validate:h19-closeout`** passes when evidence is present (retroactive closeout when **H25** is terminal).
...
- **`npm run validate:h20-closeout`** passes when evidence is present (retroactive closeout when **H25** is terminal).
...
- **`npm run validate:h21-closeout`** passes when evidence is present (retroactive closeout when **H25** is terminal).
...
- **`npm run validate:h22-closeout`** passes when evidence is present (retroactive closeout when **H25** is terminal).
...
### Horizon H25 - Terminal sustainment pin (post-H24 manifest + emergency rollback evidence)

Goal: pin the **post-H24** sustainment loop artifact with a dedicated **H25** closeout and add **machine-readable** **`rehearse:emergency-rollback`** evidence so H3 emergency rollback rehearsal stays on the same **`evidence/`** snapshot as the inner sustainment chain.
...
- **`npm run verify:sustainment-loop`**: **`run-post-h25-sustainment-loop.mjs`** runs **`run-post-h24-sustainment-loop.mjs`** then **`run:emergency-rollback-evidence`** then **`validate:h25-closeout`** → **`evidence/post-h25-sustainment-loop-*.json`**.
