...
- **`npm run validate:h11-closeout`** passes when evidence is present (stage-promotion readiness skipped when the next horizon is already completed or when validating terminal **H23**).
...
- **`npm run validate:h12-closeout`** passes when evidence is present (stage-promotion readiness skipped when the next horizon is already completed or when validating terminal **H23**).
...
- **`npm run validate:h13-closeout`** passes when evidence is present (stage-promotion readiness skipped when the next horizon is already completed or when validating terminal **H23**).
...
Goal: tie **merge-bundle readiness discipline** to executable assurance by chaining **`validate:goal-policy-file`** (full runway **H2→H23**) and **`validate:manifest-schemas`** on **`evidence/`** after the **H15** bundle, so horizon transitions stay aligned with machine-readable evidence shapes before **`validate:all`**.
...
### Horizon H23 - Terminal sustainment + evidence-gates cross-check

Goal: after **shell unified-dispatch CI** evidence, continuously cross-check **`validation-summary-*.json`** against **`failure-injection-*.txt`** via **`evidence-gates.mjs`** (required failure scenarios + P95 gate) before tenant isolation evidence, so **`validate:all`** evidence stays reconciled with failure-injection coverage.

Workstreams:

- **`npm run run:evidence-gates-evidence`**: pairs newest **`validation-summary-*.json`** + **`failure-injection-*.txt`** under **`evidence/`**; runs **`node ./scripts/evidence-gates.mjs`** with **`--require-failure-scenarios 1`** and **`--max-p95-ms`** (default **2500** via **`UNIFIED_EVIDENCE_GATES_MAX_P95_MS`**); writes **`evidence/evidence-gates-evidence-*.json`**.
- **`npm run verify:sustainment-loop`**: **`run-post-h23-sustainment-loop.mjs`** chains **`validate:horizon-status`** + **`run:h17-assurance-bundle`** + **`run:h18-assurance-bundle`** + **`run:ci-soak-slo-gate`** + **`run:unified-entrypoints-evidence`** + **`run:shell-unified-dispatch-ci-evidence`** + **`run:evidence-gates-evidence`** + **`run:tenant-isolation-evidence`** + **`validate:h23-closeout`** → **`evidence/post-h23-sustainment-loop-*.json`**.
