...
- **`npm run validate:h11-closeout`** passes when evidence is present (stage-promotion readiness skipped when the next horizon is already completed or when validating terminal **H28**).
...
- **`npm run validate:h12-closeout`** passes when evidence is present (stage-promotion readiness skipped when the next horizon is already completed or when validating terminal **H28**).
...
- **`npm run validate:h13-closeout`** passes when evidence is present (stage-promotion readiness skipped when the next horizon is already completed or when validating terminal **H28**).
...
After **H15** is marked completed, **`npm run run:h15-assurance-bundle`** remains the shell CI convergence path; terminal sustainment uses **Post-H28** below. Legacy: **`verify:sustainment-loop:h14-legacy`** … **`h6-legacy`**.

### Horizon H16 - Merge readiness policy gates (goal policy + manifest schemas)

Goal: tie **merge-bundle readiness discipline** to executable assurance by chaining **`validate:goal-policy-file`** (full runway **H2→H28**) and **`validate:manifest-schemas`** on **`evidence/`** after the **H15** bundle, so horizon transitions stay aligned with machine-readable evidence shapes before **`validate:all`**.
...
- **`npm run validate:h16-closeout`** passes when evidence is present (retroactive closeout when **H28** is terminal).
...
- **`npm run validate:h17-closeout`** passes when evidence is present (retroactive closeout when **H28** is terminal).
...
- **`npm run validate:h18-closeout`** (H18 replay when **H28** is terminal).

Exit evidence:

- **`npm run run:h18-assurance-bundle`** passes and artifact matches **`evidence/h18-assurance-bundle-*.json`**.
- **`npm run validate:h18-closeout`** passes when evidence is present (retroactive closeout when **H28** is terminal).
...
- **`npm run validate:h19-closeout`** passes when evidence is present (retroactive closeout when **H28** is terminal).
...
- **`npm run validate:h20-closeout`** passes when evidence is present (retroactive closeout when **H28** is terminal).
...
- **`npm run validate:h21-closeout`** passes when evidence is present (retroactive closeout when **H28** is terminal).
...
- **`npm run validate:h22-closeout`** passes when evidence is present (retroactive closeout when **H28** is terminal).

### Horizon H23 - Evidence-gates cross-check in sustainment (legacy terminal chain)

Goal: after **tenant isolation** evidence, continuously cross-check **`validation-summary-*.json`** + **`failure-injection-*.txt`** via **`evidence-gates.mjs`** so soak/failure-injection SLO signals stay aligned with merge and release gates.

Workstreams:

- **`npm run run:evidence-gates-evidence`**: pairs latest summary + failure report; runs **`node ./scripts/evidence-gates.mjs`**; writes **`evidence/evidence-gates-evidence-*.json`**.
- **`npm run verify:sustainment-loop:h23-legacy`**: **`run-post-h23-sustainment-loop.mjs`** adds **`run:evidence-gates-evidence`** after tenant isolation + **`validate:h23-closeout`** → **`evidence/post-h23-sustainment-loop-*.json`**.
- **`npm run validate:post-h23-sustainment-manifest`**, **`npm run verify:sustainment-loop:h22-legacy`**.

Exit evidence:

- **`npm run verify:sustainment-loop:h23-legacy`** passes when reproducing the H23 sustainment chain.
- **`npm run validate:h23-closeout`** passes when evidence is present (retroactive closeout when **H28** is terminal).

### Horizon H24 - Region failover evidence in sustainment (legacy terminal chain)

Goal: after **evidence-gates**, continuously record **machine-readable** region standby swap rehearsal evidence for H5 routing durability.

Workstreams:

- **`npm run run:region-failover-evidence`**: wraps **`npm run rehearse:region-failover`**; asserts latest **`region-failover-rehearsal-*.json`**; writes **`evidence/region-failover-evidence-*.json`**.
- **`npm run verify:sustainment-loop:h24-legacy`**: **`run-post-h24-sustainment-loop.mjs`** chains prior steps + **`validate:h24-closeout`** → **`evidence/post-h24-sustainment-loop-*.json`**.
- **`npm run validate:post-h24-sustainment-manifest`**, **`npm run verify:sustainment-loop:h23-legacy`**.

Exit evidence:

- **`npm run verify:sustainment-loop:h24-legacy`** passes when reproducing the H24 sustainment chain.
- **`npm run validate:h24-closeout`** passes when evidence is present (retroactive closeout when **H28** is terminal).

### Horizon H25 - Agent remediation evidence in sustainment (legacy terminal chain)

Goal: after **region failover** evidence, continuously record bounded **read-only** agent remediation playbook rehearsal evidence.

Workstreams:

- **`npm run run:agent-remediation-evidence`**: wraps **`npm run rehearse:agent-remediation`**; asserts latest **`agent-remediation-playbook-*.json`**; writes **`evidence/agent-remediation-evidence-*.json`**.
- **`npm run verify:sustainment-loop:h25-legacy`**: **`run-post-h25-sustainment-loop.mjs`** chains prior steps + **`validate:h25-closeout`** → **`evidence/post-h25-sustainment-loop-*.json`**.
- **`npm run validate:post-h25-sustainment-manifest`**, **`npm run verify:sustainment-loop:h24-legacy`**.

Exit evidence:

- **`npm run verify:sustainment-loop:h25-legacy`** passes when reproducing the H25 sustainment chain.
- **`npm run validate:h25-closeout`** passes when evidence is present (retroactive closeout when **H28** is terminal).

### Horizon H26 - Emergency rollback evidence in sustainment (legacy terminal chain)

Goal: after **agent remediation** evidence, continuously record **dry-run** emergency rollback rehearsal manifests so Eve-safe rollback templates stay exercised without apply.

Workstreams:

- **`npm run run:emergency-rollback-evidence`**: wraps **`npm run rehearse:emergency-rollback`**; asserts latest **`emergency-rollback-rehearsal-*.json`** (`dryRun: true`, apply skipped); writes **`evidence/emergency-rollback-evidence-*.json`**.
- **`npm run verify:sustainment-loop:h26-legacy`**: **`run-post-h26-sustainment-loop.mjs`** chains prior steps + **`validate:h26-closeout`** → **`evidence/post-h26-sustainment-loop-*.json`**.
- **`npm run validate:post-h26-sustainment-manifest`**, **`npm run verify:sustainment-loop:h25-legacy`**.

Exit evidence:

- **`npm run verify:sustainment-loop:h26-legacy`** passes when reproducing the H26 sustainment chain.
- **`npm run validate:h26-closeout`** passes when evidence is present (retroactive closeout when **H28** is terminal).

### Horizon H27 - Terminal sustainment + manifest schema gate on post-H26 loop (legacy chain)

Goal: after **emergency rollback** evidence, continuously run **`validate:manifest-schemas`** and assert the newest **`post-h26-sustainment-loop-*.json`** matches the **`post-h26-sustainment-loop`** schema so terminal sustainment manifests cannot silently drift.

Workstreams:

- **`npm run run:manifest-schemas-terminal-evidence`**: runs **`npm run validate:manifest-schemas`**; validates newest post-H26 loop JSON via **`validate-manifest-schema.mjs`**; writes **`evidence/manifest-schemas-terminal-evidence-*.json`**.
- **`npm run verify:sustainment-loop:h27-legacy`**: **`run-post-h27-sustainment-loop.mjs`** chains the post-H26 steps + **`run:manifest-schemas-terminal-evidence`** + **`validate:h27-closeout`** → **`evidence/post-h27-sustainment-loop-*.json`**.
- **`npm run validate:post-h27-sustainment-manifest`**, **`npm run verify:sustainment-loop:h26-legacy`**.
...
- **`npm run validate:h27-closeout`** passes when evidence is present (retroactive closeout when **H28** is terminal).

### Horizon H28 - Terminal sustainment + post-H27 manifest schema pin + stage-promotion sustainment evidence

Goal: after the full **post-H27** sustainment chain, pin the **post-H27** loop manifest shape under **`validate:manifest-schemas`**, then record a bounded **shadow→canary** **`check:stage-promotion-readiness`** probe (with **`--allow-horizon-mismatch`**) so cutover promotion machinery stays exercised even when **`activeHorizon`** is terminal **full**.

Workstreams:

- **`npm run run:manifest-schemas-post-h27-loop-evidence`**: runs **`npm run validate:manifest-schemas`**; validates newest **`post-h27-sustainment-loop-*.json`** via **`validate-manifest-schema.mjs`**; writes **`evidence/manifest-schemas-post-h27-loop-evidence-*.json`**.
- **`npm run run:stage-promotion-sustainment-evidence`**: runs **`check:stage-promotion-readiness`** with **`--target-stage canary --current-stage shadow --allow-horizon-mismatch`**; asserts newest **`stage-promotion-readiness-*.json`** passes schema + payload; writes **`evidence/stage-promotion-sustainment-evidence-*.json`**.
- **`npm run verify:sustainment-loop`**: **`run-post-h28-sustainment-loop.mjs`** chains **`run-post-h27-sustainment-loop.mjs`** + **`run:manifest-schemas-post-h27-loop-evidence`** + **`run:stage-promotion-sustainment-evidence`** + **`validate:h28-closeout`** → **`evidence/post-h28-sustainment-loop-*.json`**.
- **`npm run validate:post-h28-sustainment-manifest`**, **`npm run verify:sustainment-loop:h27-legacy`**.

Exit evidence:

- **`npm run verify:sustainment-loop`** passes and the newest manifest satisfies **`validate:post-h28-sustainment-manifest`**.
- **`npm run validate:h28-closeout`** passes when evidence is present (**H28** is terminal: stage-promotion readiness skipped in closeout validator).

### Post-H28 operations (terminal sustainment)

After **H28** is marked completed, use **`npm run verify:sustainment-loop`** (see `docs/MASTER_EXECUTION_CHECKLIST.md` Phase 8). Optionally **`npm run validate:post-h28-sustainment-manifest`**. Legacy prior chains: **`verify:sustainment-loop:h27-legacy`** … **`h22-legacy`** and matching **`validate:post-*-sustainment-manifest`** scripts, plus **`verify:sustainment-loop:h21-legacy`** … **`h6-legacy`**.
...
The roadmap horizons **H1–H28** are completed in `docs/HORIZON_STATUS.json`. For ongoing verification, use **`npm run verify:sustainment-loop`** and **`npm run validate:post-h28-sustainment-manifest`** (Phase 8 in `docs/MASTER_EXECUTION_CHECKLIST.md`). The steps below remain as a reference for **H2** stage-drill and promotion workflows.
