...
After **H15** is marked completed, **`npm run run:h15-assurance-bundle`** remains the shell CI convergence path; terminal sustainment uses **Post-H22** below. Legacy: **`verify:sustainment-loop:h14-legacy`** … **`h6-legacy`**.
...
### Horizon H22 - Sustainment core + tenant isolation evidence

Goal: after **shell unified-dispatch CI** evidence in the sustainment chain, continuously record **machine-readable** **`validate:tenant-isolation`** evidence so H5 tenant routing guardrails stay wired to the same **`evidence/`** snapshot as merge, drill, soak, entrypoints, and shell CI gates. **H22 closeout** is replayed from the terminal **post-H30** loop (not embedded in the **post-H22 core** manifest).
...
- **`npm run verify:sustainment-loop:h22-core`**: **`run-post-h22-sustainment-loop.mjs`** chains **`validate:horizon-status`** + **`run:h17-assurance-bundle`** + **`run:h18-assurance-bundle`** + **`run:ci-soak-slo-gate`** + **`run:unified-entrypoints-evidence`** + **`run:shell-unified-dispatch-ci-evidence`** + **`run:tenant-isolation-evidence`** → **`evidence/post-h22-sustainment-loop-*.json`** (no in-manifest H22 closeout).
- **`npm run verify:sustainment-loop:h22-legacy-terminal`**: legacy single-manifest chain including **`validate:h22-closeout`**.
...
- **`npm run verify:sustainment-loop:h22-core`** passes and the newest manifest satisfies **`validate:post-h22-sustainment-manifest`**.
- **`npm run validate:h22-closeout`** passes when run from the terminal sustainment chain (retroactive evidence alignment).

### Horizons H23–H30 - Layered terminal sustainment

Goal: extend the sustainment chain with evidence gates, region failover, agent remediation, emergency rollback, manifest schema pins, stage-promotion sustainment probe, dispatch contract fixture validation, and terminal **H30** closeout. See **`package.json`** scripts **`verify:sustainment-loop`** (terminal) and **`verify:sustainment-loop:h29-legacy`** … **`h23-legacy`**.

Exit evidence:

- **`npm run verify:sustainment-loop`** passes and **`npm run validate:post-h30-sustainment-manifest`** validates the newest **`post-h30`** manifest.

### Post-H30 operations (terminal sustainment)

After **H30** is marked completed, use **`npm run verify:sustainment-loop`** (see `docs/MASTER_EXECUTION_CHECKLIST.md` Phase 8). Optionally **`npm run validate:post-h30-sustainment-manifest`**. Legacy prior chains: **`verify:sustainment-loop:h29-legacy`** … **`h22-core`**; **`verify:sustainment-loop:h22-legacy-terminal`**.
