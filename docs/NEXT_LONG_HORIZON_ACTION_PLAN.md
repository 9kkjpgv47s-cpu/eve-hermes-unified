...
- **`npm run verify:sustainment-loop:h22-legacy`** passes when reproducing the post-H22 chain without **H23–H30** terminal slices.
...
- **`npm run verify:sustainment-loop`**: **`run-post-h30-sustainment-loop.mjs`** runs the post-H22 chain then **`run:h30-assurance-bundle`** + **`validate:h30-closeout`** → **`evidence/post-h30-sustainment-loop-*.json`**.
