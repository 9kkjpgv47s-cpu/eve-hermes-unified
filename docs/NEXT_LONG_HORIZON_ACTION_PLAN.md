...
- **`npm run validate:h11-closeout`** passes when evidence is present (stage-promotion readiness skipped when the next horizon is already completed or when validating terminal **H22**).
...
- **`npm run validate:h12-closeout`** passes when evidence is present (stage-promotion readiness skipped when the next horizon is already completed or when validating terminal **H22**).
...
- **`npm run validate:h13-closeout`** passes when evidence is present (stage-promotion readiness skipped when the next horizon is already completed or when validating terminal **H22**).
...
After **H15** is marked completed, **`npm run run:h15-assurance-bundle`** remains the shell CI convergence path; terminal sustainment uses **Post-H23** below. Legacy: **`verify:sustainment-loop:h14-legacy`** … **`h6-legacy`**.

### Horizon H16 - Horizon metadata sustainment (policy gates in H23 terminal bundle)

Goal: keep **`npm run run:h16-assurance-bundle`** as a lightweight **`validate:horizon-status`** replay path while **`validate:goal-policy-file`** (**H2→H23**) and **`validate:manifest-schemas`** run inside **`run-h23-assurance-bundle`** after **`validate:release-readiness`** / **`validate:initial-scope`** populate **`evidence/`**.

Workstreams:

- **`npm run run:h16-assurance-bundle`**: **`validate:horizon-status`** only; artifact includes **`horizonStatusPass`**.
...
- **`npm run validate:h16-closeout`** passes when evidence is present (retroactive closeout when **H23** is terminal).

Primary risks:

- **`validate:manifest-schemas`** requires existing JSON under **`evidence/`**; CI creates **`evidence/`** and **`validate:all`** plus **`run-h23-assurance-bundle`** populate manifests.

Mitigations:

- Run **`run:h23-assurance-bundle`** after **`validate:initial-scope`** so merge inputs exist before goal-policy + manifest sweep.
...
- **`npm run validate:h17-closeout`** passes when evidence is present (retroactive closeout when **H23** is terminal).
...
- **`npm run validate:h18-closeout`** (H18 replay when **H23** is terminal).

Exit evidence:

- **`npm run run:h18-assurance-bundle`** passes and artifact matches **`evidence/h18-assurance-bundle-*.json`**.
- **`npm run validate:h18-closeout`** passes when evidence is present (retroactive closeout when **H23** is terminal).
...
- **`npm run validate:h19-closeout`** passes when evidence is present (retroactive closeout when **H23** is terminal).
...
- **`npm run validate:h20-closeout`** passes when evidence is present (retroactive closeout when **H23** is terminal).
...
- **`npm run validate:h21-closeout`** passes when evidence is present (retroactive closeout when **H23** is terminal).

### Horizon H22 - Operational sub-chain + tenant isolation evidence

Goal: bundle **merge readiness**, **cutover rehearsal**, **CI soak**, **entrypoints**, **shell CI**, and **tenant isolation** evidence into **`run-h22-assurance-bundle`** so **`run-h23-assurance-bundle`** can wrap policy/manifest gates without duplicating standalone CI steps.
...
- **`npm run run:h22-assurance-bundle`**: chains **`run:h17-assurance-bundle`** + **`run:h18-assurance-bundle`** + **`run:ci-soak-slo-gate`** + **`run:unified-entrypoints-evidence`** + **`run:shell-unified-dispatch-ci-evidence`** + **`run:tenant-isolation-evidence`** → **`evidence/h22-assurance-bundle-*.json`**.
- **`npm run verify:sustainment-loop:h22-legacy`**: **`run-post-h22-sustainment-loop-legacy.mjs`** chains **`validate:horizon-status`** + the same operational steps + **`validate:h22-closeout`** → **`evidence/post-h22-sustainment-loop-*.json`** (legacy replay without **`run-h23-assurance-bundle`**).

Exit evidence:

- **`npm run run:h22-assurance-bundle`** passes when **`validate:release-readiness`**, **`validate:initial-scope`**, and **`npm run build`** (via **`validate:all`**) have populated **`evidence/`**.
- **`npm run validate:h22-closeout`** passes when evidence is present (retroactive closeout when **H23** is terminal).

### Horizon H23 - Goal policy + manifest schemas + slim H22 operational chain

Goal: consolidate **merge readiness policy** (**`validate:goal-policy-file`** full runway **H2→H30**) and **manifest schema sweep** with the slim **H22** operational chain into **`run-h23-assurance-bundle`** (**H22** drops unified-entrypoints + tenant-isolation evidence wrappers — those run under **H24**).

Workstreams:

- **`npm run run:h23-assurance-bundle`**: **`validate:goal-policy-file`** + **`validate:manifest-schemas`** + **`run:h22-assurance-bundle`** → **`evidence/h23-assurance-bundle-*.json`**.
- **`npm run verify:sustainment-loop:h23-legacy`**: **`run-post-h23-sustainment-loop-legacy.mjs`** chains **`validate:horizon-status`** + **`run:h23-assurance-bundle`** + **`validate:h23-closeout`** → **`evidence/post-h23-sustainment-loop-*.json`**.
...
- **`npm run verify:sustainment-loop:h23-legacy`** passes and the newest manifest satisfies **`validate:post-h23-sustainment-manifest`** (when **H30** is terminal, **H23** is a legacy replay).
...
Goal: fold **unified entrypoints**, **tenant isolation**, and **region failover rehearsal** into **`run-h24-assurance-bundle`** so **`unified-ci`** does not duplicate standalone pre-build steps; **`verify:sustainment-loop`** advances to **post-H30**; legacy **post-H24** replay via **`verify:sustainment-loop:h24-legacy`**.
...
Goal: fold **`run-h6-assurance-bundle`** + **`run-h16-assurance-bundle`** into **`run-h25-assurance-bundle`** so **`unified-ci`** does not duplicate standalone horizon-metadata steps before **`validate:all`**; **`verify:sustainment-loop`** advances to **post-H30**; legacy **post-H25** replay via **`verify:sustainment-loop:h25-legacy`**.
...
- **`npm run verify:sustainment-loop:h25-legacy`**: **`run-post-h25-sustainment-loop-legacy.mjs`** chains **`validate:horizon-status`** + **`run:h25-assurance-bundle`** + **`validate:h25-closeout`** → **`evidence/post-h25-sustainment-loop-*.json`**.
...
### Horizon H26 - Stage promotion readiness + H25 terminal chain

Goal: fold **`check:stage-promotion-readiness`** (canary target, **`--allow-horizon-mismatch`**) into **`run-h26-assurance-bundle`** after **`run-h25-assurance-bundle`** so **`unified-ci`** does not duplicate a standalone stage gate; **`verify:sustainment-loop`** advances to **post-H30**; legacy **post-H26** replay via **`verify:sustainment-loop:h26-legacy`**.

Workstreams:

- **`npm run run:h26-assurance-bundle`**: **`run-h25-assurance-bundle`** + **`check:stage-promotion-readiness`** (same argv as CI) → **`evidence/h26-assurance-bundle-*.json`**.
- **`npm run verify:sustainment-loop:h26-legacy`**: **`run-post-h26-sustainment-loop-legacy.mjs`** chains **`validate:horizon-status`** + **`run:h26-assurance-bundle`** + **`validate:h26-closeout`** → **`evidence/post-h26-sustainment-loop-*.json`**.
...
### Horizon H27 - Horizon status schema gate + H26 terminal chain

Goal: fold **`validate:horizon-status`** into **`run-h27-assurance-bundle`** before **`run-h26-assurance-bundle`** so **`unified-ci`** does not duplicate a standalone horizon-metadata validation step; **`verify:sustainment-loop`** advances to **post-H30**; legacy **post-H27** replay via **`verify:sustainment-loop:h27-legacy`**.

Workstreams:

- **`npm run run:h27-assurance-bundle`**: **`validate:horizon-status`** + **`run-h26-assurance-bundle`** → **`evidence/h27-assurance-bundle-*.json`**.
- **`npm run verify:sustainment-loop:h27-legacy`**: **`run-post-h27-sustainment-loop-legacy.mjs`** chains **`run:h27-assurance-bundle`** + **`validate:h27-closeout`** → **`evidence/post-h27-sustainment-loop-*.json`**.
...
### Horizon H28 - Initial scope gate + H27 terminal chain

Goal: fold **`validate:initial-scope`** into **`run-h28-assurance-bundle`** before **`run-h27-assurance-bundle`** so **`unified-ci`** does not duplicate a standalone initial-scope step after **`validate:release-readiness`** (until folded under **H29**); **`verify:sustainment-loop`** advances to **post-H30**; legacy **post-H28** replay via **`verify:sustainment-loop:h28-legacy`**.

Workstreams:

- **`npm run run:h28-assurance-bundle`**: **`validate:initial-scope`** + **`run-h27-assurance-bundle`** → **`evidence/h28-assurance-bundle-*.json`**.
- **`npm run verify:sustainment-loop:h28-legacy`**: **`run-post-h28-sustainment-loop-legacy.mjs`** chains **`run:h28-assurance-bundle`** + **`validate:h28-closeout`** → **`evidence/post-h28-sustainment-loop-*.json`**.
...
### Horizon H29 - Release readiness gate + H28 terminal chain

Goal: fold **`validate:release-readiness`** into **`run-h29-assurance-bundle`** before **`run-h28-assurance-bundle`** so **`unified-ci`** does not duplicate a standalone release-readiness step after **`validate:all`** (until folded under **H30**); **`verify:sustainment-loop`** advances to **post-H30**; legacy **post-H29** replay via **`verify:sustainment-loop:h29-legacy`**.

Workstreams:

- **`npm run run:h29-assurance-bundle`**: **`validate:release-readiness`** (same env as **`run-h30-assurance-bundle`**) + **`run-h28-assurance-bundle`** → **`evidence/h29-assurance-bundle-*.json`**.
- **`npm run verify:sustainment-loop:h29-legacy`**: **`run-post-h29-sustainment-loop-legacy.mjs`** chains **`run:h29-assurance-bundle`** + **`validate:h29-closeout`** → **`evidence/post-h29-sustainment-loop-*.json`**.
...
### Horizon H30 - Full validation chain (validate:all) + H29 terminal chain

Goal: fold **`validate:all`** into **`run-h30-assurance-bundle`** before **`run-h29-assurance-bundle`** so **`unified-ci`** does not duplicate standalone check/test/build/full-validation steps; terminal sustainment chains **`run:h30-assurance-bundle`** + **`validate:h30-closeout`**.

Workstreams:

- **`npm run run:h30-assurance-bundle`**: **`validate:all`** (same router env as **`unified-ci`**) + **`run-h29-assurance-bundle`** → **`evidence/h30-assurance-bundle-*.json`** (newest **`validation-summary-*.json`** must report **`gates.passed`**).
- **`npm run verify:sustainment-loop`**: **`run-post-h30-sustainment-loop.mjs`** chains **`run:h30-assurance-bundle`** + **`validate:h30-closeout`** → **`evidence/post-h30-sustainment-loop-*.json`**.
...
After **H30** is marked completed, use **`npm run verify:sustainment-loop`** (see `docs/MASTER_EXECUTION_CHECKLIST.md` Phase 8). Optionally **`npm run validate:post-h30-sustainment-manifest`**. Legacy prior chains: **`verify:sustainment-loop:h29-legacy`** / **`validate:post-h29-sustainment-manifest`**, **`verify:sustainment-loop:h28-legacy`** … **`h6-legacy`**.
