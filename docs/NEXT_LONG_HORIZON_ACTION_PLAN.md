...
- **`npm run verify:sustainment-loop:h22-legacy`** passes when reproducing the post-H22 chain without **H23/H24** terminal slices.
...
### Horizon H24 - Terminal sustainment + horizon-status recheck

Goal: after **H23** manifest-schema bundling passes, re-validate **`docs/HORIZON_STATUS.json`** end-to-end so horizon metadata and embedded goal policies stay consistent with the full terminal chain.
...
- **`npm run verify:sustainment-loop`**: **`run-post-h24-sustainment-loop.mjs`** runs the post-H22 chain then **`run:h24-assurance-bundle`** + **`validate:h24-closeout`** → **`evidence/post-h24-sustainment-loop-*.json`**.
