...
### Horizon H23 - Terminal sustainment pin (post-H22 manifest + region failover evidence)

Goal: pin the **post-H22** sustainment loop artifact with a dedicated **H23** closeout and add **machine-readable** **`rehearse:region-failover`** evidence so H5 region standby routing stays on the same **`evidence/`** snapshot as the inner sustainment chain.
...
- **`npm run verify:sustainment-loop`**: **`run-post-h23-sustainment-loop.mjs`** runs **`run-post-h22-sustainment-loop.mjs`** then **`run:region-failover-evidence`** then **`validate:h23-closeout`** → **`evidence/post-h23-sustainment-loop-*.json`**.
...
After **H23** is marked completed, use **`npm run verify:sustainment-loop`** (see `docs/MASTER_EXECUTION_CHECKLIST.md` Phase 8). Optionally **`npm run validate:post-h23-sustainment-manifest`**. Legacy **H22-only** inner chain: **`verify:sustainment-loop:h22-legacy`** / **`validate:post-h22-sustainment-manifest`**. Older: **`verify:sustainment-loop:h21-legacy`** / **`validate:post-h21-sustainment-manifest`**, **`verify:sustainment-loop:h20-legacy`** … **`h6-legacy`**.
