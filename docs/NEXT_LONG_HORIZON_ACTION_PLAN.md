...
- **`npm run validate:h11-closeout`** passes when evidence is present (stage-promotion readiness skipped when the next horizon is already completed or when validating terminal **H29**).
...
- **`npm run validate:h12-closeout`** passes when evidence is present (stage-promotion readiness skipped when the next horizon is already completed or when validating terminal **H29**).
...
- **`npm run validate:h13-closeout`** passes when evidence is present (stage-promotion readiness skipped when the next horizon is already completed or when validating terminal **H29**).
...
After **H15** is marked completed, **`npm run run:h15-assurance-bundle`** remains the shell CI convergence path; terminal sustainment uses **Post-H29** below. Legacy: **`verify:sustainment-loop:h14-legacy`** … **`h6-legacy`**.

### Horizon H16 - Merge readiness policy gates (goal policy + manifest schemas)

Goal: tie **merge-bundle readiness discipline** to executable assurance by chaining **`validate:goal-policy-file`** (full runway **H2→H29**) and **`validate:manifest-schemas`** on **`evidence/`** after the **H15** bundle, so horizon transitions stay aligned with machine-readable evidence shapes before **`validate:all`**.
...
- **`npm run validate:h16-closeout`** passes when evidence is present (retroactive closeout when **H29** is terminal).
...
- **`npm run validate:h17-closeout`** passes when evidence is present (retroactive closeout when **H29** is terminal).
...
- **`npm run validate:h18-closeout`** (H18 replay when **H29** is terminal).

Exit evidence:

- **`npm run run:h18-assurance-bundle`** passes and artifact matches **`evidence/h18-assurance-bundle-*.json`**.
- **`npm run validate:h18-closeout`** passes when evidence is present (retroactive closeout when **H29** is terminal).
...
- **`npm run validate:h19-closeout`** passes when evidence is present (retroactive closeout when **H29** is terminal).
...
- **`npm run validate:h20-closeout`** passes when evidence is present (retroactive closeout when **H29** is terminal).
...
- **`npm run validate:h21-closeout`** passes when evidence is present (retroactive closeout when **H29** is terminal).
...
- **`npm run validate:h22-closeout`** passes when evidence is present (retroactive closeout when **H29** is terminal).
...
- **`npm run validate:h23-closeout`** passes when evidence is present (retroactive closeout when **H29** is terminal).
...
- **`npm run validate:h24-closeout`** passes when evidence is present (retroactive closeout when **H29** is terminal).
...
- **`npm run validate:h25-closeout`** passes when evidence is present (retroactive closeout when **H29** is terminal).
...
- **`npm run validate:h26-closeout`** passes when evidence is present (retroactive closeout when **H29** is terminal).
...
- **`npm run validate:h27-closeout`** passes when evidence is present (retroactive closeout when **H29** is terminal).
...
- **`npm run validate:h28-closeout`** passes when evidence is present (retroactive closeout when **H29** is terminal).

### Horizon H29 - Terminal sustainment pin (post-H28 manifest + evidence-gates gate evidence)

Goal: pin the **post-H28** sustainment loop artifact with a dedicated **H29** closeout and add **machine-readable** **`validate:evidence-gates`** evidence so evidence-gates aggregation stays on the same **`evidence/`** snapshot as the inner sustainment chain.
...
- **`npm run verify:sustainment-loop`**: **`run-post-h29-sustainment-loop.mjs`** runs **`run-post-h28-sustainment-loop.mjs`** then **`run:evidence-gates-evidence`** then **`validate:h29-closeout`** → **`evidence/post-h29-sustainment-loop-*.json`**.
...
- **`npm run verify:sustainment-loop`** passes and the newest manifest satisfies **`validate:post-h29-sustainment-manifest`**.
- **`npm run validate:h29-closeout`** passes when evidence is present (**H29** is terminal: stage-promotion readiness skipped in closeout validator).

### Post-H29 operations (terminal sustainment)

After **H29** is marked completed, use **`npm run verify:sustainment-loop`** (see `docs/MASTER_EXECUTION_CHECKLIST.md` Phase 8). Optionally **`npm run validate:post-h29-sustainment-manifest`**. Legacy **H28-only** inner chain: **`verify:sustainment-loop:h28-legacy`** / **`validate:post-h28-sustainment-manifest`**. Older: **`verify:sustainment-loop:h27-legacy`** / **`validate:post-h27-sustainment-manifest`**, **`verify:sustainment-loop:h26-legacy`** … **`h6-legacy`**.
...
The roadmap horizons **H1–H29** are completed in `docs/HORIZON_STATUS.json`. For ongoing verification, use **`npm run verify:sustainment-loop`** and **`npm run validate:post-h29-sustainment-manifest`** (Phase 8 in `docs/MASTER_EXECUTION_CHECKLIST.md`). The steps below remain as a reference for **H2** stage-drill and promotion workflows.
