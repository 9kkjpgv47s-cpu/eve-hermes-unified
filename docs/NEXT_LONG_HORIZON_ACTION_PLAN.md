...
### Horizon H19 - Terminal sustainment + CI soak SLO drift gate

Goal: after **H18** rehearsal passes, continuously re-run the **CI soak SLO drift gate** inside the same **post-sustainment** chain so mixed-traffic soak thresholds stay enforced alongside merge and drill evidence.
...
- **`npm run verify:sustainment-loop:h19-legacy`** passes and the newest manifest satisfies **`validate:post-h19-sustainment-manifest`**.
- **`npm run validate:h19-closeout`** passes when evidence is present (**H19** inner sustainment: stage-promotion readiness skipped when no downstream horizon in sequence for that closeout).

### Horizon H20 - Terminal sustainment pin (post-H19 manifest)

Goal: pin the **post-H19** sustainment loop artifact with a dedicated **H20** closeout so operators have a single manifest proving both the inner chain and the pin gate.

Workstreams:

- **`npm run verify:sustainment-loop`**: **`run-post-h20-sustainment-loop.mjs`** runs **`run-post-h19-sustainment-loop.mjs`** then **`validate:h20-closeout`** → **`evidence/post-h20-sustainment-loop-*.json`**.
- **`npm run validate:post-h20-sustainment-manifest`**.
