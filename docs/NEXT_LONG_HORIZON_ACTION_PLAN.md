...
- **`npm run verify:sustainment-loop:h22-legacy`** passes when reproducing the post-H22 chain without **H23–H26** terminal slices.
...
### Horizon H26 - Terminal sustainment + horizon-status recheck

Goal: after **H25** manifest-schema bundling passes, re-validate **`docs/HORIZON_STATUS.json`** end-to-end so horizon metadata and embedded goal policies stay consistent with the full terminal chain.
...
- **`npm run verify:sustainment-loop`**: **`run-post-h26-sustainment-loop.mjs`** runs the post-H22 chain then **`run:h26-assurance-bundle`** + **`validate:h26-closeout`** → **`evidence/post-h26-sustainment-loop-*.json`**.
