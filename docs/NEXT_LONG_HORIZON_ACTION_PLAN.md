...
### Horizon H27 - Terminal sustainment + manifest schema gate on post-H26 loop
...
- **`npm run verify:sustainment-loop`**: **`run-post-h27-sustainment-loop.mjs`** chains the post-H26 steps + **`run:manifest-schemas-terminal-evidence`** + **`validate:h27-closeout`** → **`evidence/post-h27-sustainment-loop-*.json`**.
