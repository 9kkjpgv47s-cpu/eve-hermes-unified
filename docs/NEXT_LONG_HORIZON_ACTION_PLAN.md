...
- **`npm run validate:h11-closeout`** passes when evidence is present (stage-promotion readiness skipped when the next horizon is already completed or when validating terminal **H30**).
...
- **`npm run validate:h12-closeout`** passes when evidence is present (stage-promotion readiness skipped when the next horizon is already completed or when validating terminal **H30**).
...
- **`npm run validate:h13-closeout`** passes when evidence is present (stage-promotion readiness skipped when the next horizon is already completed or when validating terminal **H30**).
...
After **H15** is marked completed, **`npm run run:h15-assurance-bundle`** remains the shell CI convergence path; terminal sustainment uses **Post-H30** below. Legacy: **`verify:sustainment-loop:h14-legacy`** … **`h6-legacy`**.

### Horizon H16 - Merge readiness policy gates (goal policy + manifest schemas)

Goal: tie **merge-bundle readiness discipline** to executable assurance by chaining **`validate:goal-policy-file`** (full runway **H2→H30**) and **`validate:manifest-schemas`** on **`evidence/`** after the **H15** bundle, so horizon transitions stay aligned with machine-readable evidence shapes before **`validate:all`**.
...
- **`npm run validate:h16-closeout`** passes when evidence is present (retroactive closeout when **H30** is terminal).
...
- **`npm run validate:h17-closeout`** passes when evidence is present (retroactive closeout when **H30** is terminal).
...
- **`npm run validate:h18-closeout`** (H18 replay when **H30** is terminal).

Exit evidence:

- **`npm run run:h18-assurance-bundle`** passes and artifact matches **`evidence/h18-assurance-bundle-*.json`**.
- **`npm run validate:h18-closeout`** passes when evidence is present (retroactive closeout when **H30** is terminal).
...
- **`npm run validate:h19-closeout`** passes when evidence is present (retroactive closeout when **H30** is terminal).
...
- **`npm run validate:h20-closeout`** passes when evidence is present (retroactive closeout when **H30** is terminal).
...
- **`npm run validate:h21-closeout`** passes when evidence is present (retroactive closeout when **H30** is terminal).

### Horizon H22 - Tenant isolation evidence in sustainment (legacy inner chain)
...
- **`npm run verify:sustainment-loop:h22-legacy`** passes when reproducing the H22-only sustainment chain.
- **`npm run validate:h22-closeout`** passes when evidence is present (retroactive closeout when **H30** is terminal).

### Horizon H23 - Region failover evidence in sustainment (legacy inner chain)

Goal: after **tenant isolation** evidence in the sustainment chain, continuously record **machine-readable** **`rehearse:region-failover`** evidence so H5 region standby routing stays on the same **`evidence/`** snapshot as merge, drill, soak, entrypoints, shell CI, and tenant gates.

Workstreams:

- **`npm run run:region-failover-evidence`**: wraps **`npm run rehearse:region-failover`**; writes **`evidence/region-failover-evidence-*.json`**.
- **`npm run verify:sustainment-loop:h23-legacy`**: **`run-post-h23-sustainment-loop.mjs`** runs **`run-post-h22-sustainment-loop.mjs`** then **`run:region-failover-evidence`** then **`validate:h23-closeout`** → **`evidence/post-h23-sustainment-loop-*.json`**.
...
- **`npm run verify:sustainment-loop:h23-legacy`** passes when reproducing the H23-only sustainment chain.
- **`npm run validate:h23-closeout`** passes when evidence is present (retroactive closeout when **H30** is terminal).

### Horizon H24 - Agent remediation evidence in sustainment (legacy inner chain)

Goal: after **region failover** evidence in the sustainment chain, continuously record **machine-readable** **`rehearse:agent-remediation`** evidence so H5 bounded remediation stays on the same **`evidence/`** snapshot as merge, drill, soak, entrypoints, shell CI, tenant, and region gates.

Workstreams:

- **`npm run run:agent-remediation-evidence`**: wraps **`npm run rehearse:agent-remediation`**; writes **`evidence/agent-remediation-evidence-*.json`**.
- **`npm run verify:sustainment-loop:h24-legacy`**: **`run-post-h24-sustainment-loop.mjs`** runs **`run-post-h23-sustainment-loop.mjs`** then **`run:agent-remediation-evidence`** then **`validate:h24-closeout`** → **`evidence/post-h24-sustainment-loop-*.json`**.
...
- **`npm run verify:sustainment-loop:h24-legacy`** passes when reproducing the H24-only sustainment chain.
- **`npm run validate:h24-closeout`** passes when evidence is present (retroactive closeout when **H30** is terminal).

### Horizon H25 - Emergency rollback evidence in sustainment (legacy inner chain)

Goal: after **agent remediation** evidence in the sustainment chain, continuously record **machine-readable** **`rehearse:emergency-rollback`** evidence so H3 emergency rollback rehearsal stays on the same **`evidence/`** snapshot as merge, drill, soak, entrypoints, shell CI, tenant, region, and remediation gates.

Workstreams:

- **`npm run run:emergency-rollback-evidence`**: wraps **`npm run rehearse:emergency-rollback`**; writes **`evidence/emergency-rollback-evidence-*.json`**.
- **`npm run verify:sustainment-loop:h25-legacy`**: **`run-post-h25-sustainment-loop.mjs`** runs **`run-post-h24-sustainment-loop.mjs`** then **`run:emergency-rollback-evidence`** then **`validate:h25-closeout`** → **`evidence/post-h25-sustainment-loop-*.json`**.
...
- **`npm run verify:sustainment-loop:h25-legacy`** passes when reproducing the H25-only sustainment chain.
- **`npm run validate:h25-closeout`** passes when evidence is present (retroactive closeout when **H30** is terminal).

### Horizon H26 - Failure-injection evidence in sustainment (legacy inner chain)

Goal: after **emergency rollback** evidence in the sustainment chain, continuously record **machine-readable** **`validate:failure-injection`** evidence so bounded failure-injection smoke stays on the same **`evidence/`** snapshot as merge, drill, soak, entrypoints, shell CI, tenant, region, remediation, and rollback gates.

Workstreams:

- **`npm run run:failure-injection-evidence`**: wraps **`npm run validate:failure-injection`**; writes **`evidence/failure-injection-evidence-*.json`**.
- **`npm run verify:sustainment-loop:h26-legacy`**: **`run-post-h26-sustainment-loop.mjs`** runs **`run-post-h25-sustainment-loop.mjs`** then **`run:failure-injection-evidence`** then **`validate:h26-closeout`** → **`evidence/post-h26-sustainment-loop-*.json`**.
...
- **`npm run verify:sustainment-loop:h26-legacy`** passes when reproducing the H26-only sustainment chain.
- **`npm run validate:h26-closeout`** passes when evidence is present (retroactive closeout when **H30** is terminal).

### Horizon H27 - Regression-Eve evidence in sustainment (legacy inner chain)

Goal: after **failure-injection** evidence in the sustainment chain, continuously record **machine-readable** **`validate:regression-eve`** evidence so Eve-primary regression rehearsal stays on the same **`evidence/`** snapshot as merge, drill, soak, entrypoints, shell CI, tenant, region, remediation, rollback, and resilience gates.

Workstreams:

- **`npm run run:regression-eve-evidence`**: wraps **`npm run validate:regression-eve`**; writes **`evidence/regression-eve-evidence-*.json`**.
- **`npm run verify:sustainment-loop:h27-legacy`**: **`run-post-h27-sustainment-loop.mjs`** runs **`run-post-h26-sustainment-loop.mjs`** then **`run:regression-eve-evidence`** then **`validate:h27-closeout`** → **`evidence/post-h27-sustainment-loop-*.json`**.
...
- **`npm run verify:sustainment-loop:h27-legacy`** passes when reproducing the H27-only sustainment chain.
- **`npm run validate:h27-closeout`** passes when evidence is present (retroactive closeout when **H30** is terminal).

### Horizon H28 - Evidence-summary evidence in sustainment (legacy inner chain)

Goal: after **regression-Eve** evidence in the sustainment chain, continuously record **machine-readable** **`validate:evidence-summary`** evidence so validation-summary aggregation stays on the same **`evidence/`** snapshot as merge, drill, soak, entrypoints, shell CI, tenant, region, remediation, rollback, resilience, and routing gates.

Workstreams:

- **`npm run run:evidence-summary-evidence`**: wraps **`npm run validate:evidence-summary`**; writes **`evidence/evidence-summary-evidence-*.json`**.
- **`npm run verify:sustainment-loop:h28-legacy`**: **`run-post-h28-sustainment-loop.mjs`** runs **`run-post-h27-sustainment-loop.mjs`** then **`run:evidence-summary-evidence`** then **`validate:h28-closeout`** → **`evidence/post-h28-sustainment-loop-*.json`**.
...
- **`npm run verify:sustainment-loop:h28-legacy`** passes when reproducing the H28-only sustainment chain.
- **`npm run validate:h28-closeout`** passes when evidence is present (retroactive closeout when **H30** is terminal).

### Horizon H29 - Evidence-gates evidence in sustainment (legacy inner chain)

Goal: after **evidence-summary** evidence in the sustainment chain, continuously record **machine-readable** **`validate:evidence-gates`** evidence so evidence-gates aggregation stays on the same **`evidence/`** snapshot as merge, drill, soak, entrypoints, shell CI, tenant, region, remediation, rollback, resilience, routing, and validation-summary gates.

Workstreams:

- **`npm run run:evidence-gates-evidence`**: wraps **`npm run validate:evidence-gates`** ( **`invoke-evidence-gates.mjs`** selects newest **`validation-summary-*.json`** + **`failure-injection-*`** under **`evidence/`** ); writes **`evidence/evidence-gates-evidence-*.json`**.
- **`npm run verify:sustainment-loop:h29-legacy`**: **`run-post-h29-sustainment-loop.mjs`** runs **`run-post-h28-sustainment-loop.mjs`** then **`run:evidence-gates-evidence`** then **`validate:h29-closeout`** → **`evidence/post-h29-sustainment-loop-*.json`**.
...
- **`npm run verify:sustainment-loop:h29-legacy`** passes when reproducing the H29-only sustainment chain.
- **`npm run validate:h29-closeout`** passes when evidence is present (retroactive closeout when **H30** is terminal).

### Horizon H30 - Terminal sustainment pin (post-H29 manifest + horizon-status gate evidence)

Goal: pin the **post-H29** sustainment loop artifact with a dedicated **H30** closeout and add **machine-readable** **`validate:horizon-status`** evidence so **`docs/HORIZON_STATUS.json`** schema and metadata gates stay on the same **`evidence/`** snapshot as the inner sustainment chain.

Workstreams:

- **`npm run run:horizon-status-evidence`**: runs **`validate-horizon-status.mjs`** (same gate as **`npm run validate:horizon-status`**) and writes **`evidence/horizon-status-evidence-*.json`**.
- **`npm run verify:sustainment-loop`**: **`run-post-h30-sustainment-loop.mjs`** runs **`run-post-h29-sustainment-loop.mjs`** then **`run:horizon-status-evidence`** then **`validate:h30-closeout`** → **`evidence/post-h30-sustainment-loop-*.json`**.
...
- **`npm run verify:sustainment-loop`** passes and the newest manifest satisfies **`validate:post-h30-sustainment-manifest`**.
- **`npm run validate:h30-closeout`** passes when evidence is present (**H30** is terminal: stage-promotion readiness skipped in closeout validator).

### Post-H30 operations (terminal sustainment)

After **H30** is marked completed, use **`npm run verify:sustainment-loop`** (see `docs/MASTER_EXECUTION_CHECKLIST.md` Phase 8). Optionally **`npm run validate:post-h30-sustainment-manifest`**. Legacy **H29-only** inner chain: **`verify:sustainment-loop:h29-legacy`** / **`validate:post-h29-sustainment-manifest`**. Older: **`verify:sustainment-loop:h28-legacy`** / **`validate:post-h28-sustainment-manifest`**, **`verify:sustainment-loop:h27-legacy`** … **`h6-legacy`**.
...
The roadmap horizons **H1–H30** are completed in `docs/HORIZON_STATUS.json`. For ongoing verification, use **`npm run verify:sustainment-loop`** and **`npm run validate:post-h30-sustainment-manifest`** (Phase 8 in `docs/MASTER_EXECUTION_CHECKLIST.md`). The steps below remain as a reference for **H2** stage-drill and promotion workflows.
