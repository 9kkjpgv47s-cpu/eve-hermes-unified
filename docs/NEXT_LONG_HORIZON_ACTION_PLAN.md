...
### Horizon H26 - Terminal sustainment pin (post-H25 manifest + failure-injection evidence)

Goal: pin the **post-H25** sustainment loop artifact with a dedicated **H26** closeout and add **machine-readable** **`validate:failure-injection`** evidence so bounded failure-injection smoke stays on the same **`evidence/`** snapshot as the inner sustainment chain.
...
- **`npm run verify:sustainment-loop`**: **`run-post-h26-sustainment-loop.mjs`** runs **`run-post-h25-sustainment-loop.mjs`** then **`run:failure-injection-evidence`** then **`validate:h26-closeout`** → **`evidence/post-h26-sustainment-loop-*.json`**.
...
After **H26** is marked completed, use **`npm run verify:sustainment-loop`** (see `docs/MASTER_EXECUTION_CHECKLIST.md` Phase 8). Optionally **`npm run validate:post-h26-sustainment-manifest`**. Legacy **H25-only** inner chain: **`verify:sustainment-loop:h25-legacy`** / **`validate:post-h25-sustainment-manifest`**. Older: **`verify:sustainment-loop:h24-legacy`** / **`validate:post-h24-sustainment-manifest`**, **`verify:sustainment-loop:h23-legacy`** … **`h6-legacy`**.
