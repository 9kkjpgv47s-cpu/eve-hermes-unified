...
- `--goal-policy-file <path>` to load transition policies from a dedicated policy document
  ...
  - `--goal-policy-file-validation-until-horizon <H3|H4|H5|H6|H7|H8|H9|H10>` to set validation transition scope
...
- `--require-goal-policy-coverage` to require machine-checkable transition policy coverage before promotion
  - default scope checks from source horizon through `--goal-policy-coverage-until-horizon` (default `H10`)
  - set `--goal-policy-coverage-until-horizon H10` to require policy coverage through remaining horizons (e.g., H2->H3->H4->H5->H6->H7->H8->H9->H10)
  - add `--required-policy-transitions "H2->H3,H3->H4,H4->H5,H5->H6,H6->H7,H7->H8,H8->H9,H9->H10"` for explicit transition sets
...
- `--require-goal-policy-readiness-audit` to run `audit:goal-policy-readiness` as an explicit promotion gate
  ...
  - add `--goal-policy-readiness-audit-max-target-horizon <H3|H4|H5|H6|H7|H8|H9|H10>` (alias of `--goal-policy-readiness-audit-until-horizon`) to define readiness-audit transition window
