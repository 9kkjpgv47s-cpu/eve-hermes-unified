...
const HORIZON_SEQUENCE = ["H1", "H2", "H3", "H4", "H5", "H6", "H7", "H8", "H9", "H10", "H11", "H12", "H13", "H14", "H15", "H16", "H17", "H18", "H19", "H20", "H21", "H22", "H23", "H24", "H25", "H26", "H27", "H28", "H29", "H30", "H31", "H32", "H33", "H34", "H35", "H36", "H37", "H38", "H39", "H40", "H41", "H42"];
...
function parseArgs(argv) {
  const options = {
    ...
    requiredPolicyKey: "",
    out: "",
    requireTaggedRequirements: false,
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--horizon-status-file") {
      ...
      index += 1;
    } else if (arg === "--required-policy-key") {
      options.requiredPolicyKey = value ?? "";
      index += 1;
    } else if (arg === "--out") {
      options.out = value ?? "";
      index += 1;
    ...
    } else if (arg === "--allow-untagged-policies") {
...
function buildTransitions(sourceHorizon, untilHorizon) {
  const sourceIndex = HORIZON_SEQUENCE.indexOf(sourceHorizon);
  const untilIndex = HORIZON_SEQUENCE.indexOf(untilHorizon);
  if (sourceIndex < 0 || untilIndex < 0 || untilIndex <= sourceIndex) {
    return [];
  }
  const transitions = [];
  for (let index = sourceIndex; index < untilIndex; index += 1) {
    const current = HORIZON_SEQUENCE[index];
    const next = HORIZON_SEQUENCE[index + 1];
...
async function main() {
  ...
  if (isNonEmptyString(requiredPolicyKey) && !requiredPolicyTransitions.includes(requiredPolicyKey)) {
    requiredPolicyTransitions.push(requiredPolicyKey);
  }
  const transitions =
    requiredPolicyTransitions.length > 0
      ? requiredPolicyTransitions
      : windowTransitions;
  ...
  const outPath = path.resolve(
    options.out ||
      path.join(
        process.cwd(),
        "evidence",
        `goal-policy-coverage-${sourceHorizon || "unknown"}-to-${maxTargetHorizon || "unknown"}-${stamp()}.json`,
      ),
  );
  ...
  const taggedRequirementCount = Object.values(transitionChecks).reduce((count, check) => {
    return count + (check.hasTaggedRequirements ? Object.keys(check.taggedRequirements).length : 0);
  }, 0);
  ...
  if (!payload.pass) {
    process.stderr.write(`Goal policy coverage check failed:\n- ${failures.join("\n- ")}\n`);
    process.exitCode = 2;
  }
}
