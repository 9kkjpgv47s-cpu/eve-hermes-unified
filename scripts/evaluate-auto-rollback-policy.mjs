...
function evaluateThresholds(stage, metrics, thresholds) {
  ...
  if (
    ...
    reasons.push(
      `policy_failure_rate_above_threshold:${metrics.policyFailureRate}>${maxPolicyFailureRate}`,
    );
  }
...
async function main() {
  ...
  const rateCritical = thresholdReasons.some(
    (reason) =>
      reason.startsWith("dispatch_failure_rate_above_threshold:") ||
      reason.startsWith("policy_failure_rate_above_threshold:"),
  );
  const stageRequiresRollback =
    stage === "shadow"
      ? false
      : stage === "canary"
        ? rateCritical || (thresholdReasons.length > 0 && metrics.successRate < 0.98)
