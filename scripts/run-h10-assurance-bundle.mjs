#!/usr/bin/env node
/**
 * Horizon H10 assurance bundle: H9 gates plus dispatch durability queue retention proof.
 *
...
const evidenceDir = process.env.H10_ASSURANCE_EVIDENCE_DIR ?? path.join(root, "evidence");
mkdirSync(evidenceDir, { recursive: true });
const stamp = new Date().toISOString().replace(/[-:]/g, "").replace(/\..+$/, "");
const outPath =
  process.env.H10_ASSURANCE_OUT ?? path.join(evidenceDir, `h10-assurance-bundle-${stamp}.json`);
...
const payload = {
  ...
  steps: [horizonStatus, auditRotation, capabilityPolicyAudit, memoryAtomic, durabilityRetention],
};
