#!/usr/bin/env node
/**
 * Horizon H31 assurance bundle: H30 gates plus final horizon-status re-validation after the
...
const h30Bundle = runStep("run_h30_assurance_bundle", [
  process.execPath,
  path.join(root, "scripts/run-h30-assurance-bundle.mjs"),
]);
...
const payload = {
  ...
  horizon: "H31",
  pass: h30Bundle.pass && horizonStatusRecheck.pass,
  checks: {
    h30AssuranceBundlePass: h30Bundle.pass,
    horizonStatusRecheckPass: horizonStatusRecheck.pass,
  },
  steps: [h30Bundle, horizonStatusRecheck],
};
