#!/usr/bin/env node
/**
 * Horizon H33 assurance bundle: H32 gates plus final horizon-status re-validation after the
...
const h32Bundle = runStep("run_h32_assurance_bundle", [
  process.execPath,
  path.join(root, "scripts/run-h32-assurance-bundle.mjs"),
]);
...
const payload = {
  ...
  horizon: "H33",
  pass: h32Bundle.pass && horizonStatusRecheck.pass,
  checks: {
    h32AssuranceBundlePass: h32Bundle.pass,
    horizonStatusRecheckPass: horizonStatusRecheck.pass,
  },
  steps: [h32Bundle, horizonStatusRecheck],
};
