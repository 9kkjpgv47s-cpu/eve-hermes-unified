#!/usr/bin/env node
/**
 * Horizon H7 assurance bundle: horizon metadata plus dispatch audit log rotation unit proof.
 *
...
const evidenceDir = process.env.H7_ASSURANCE_EVIDENCE_DIR ?? path.join(root, "evidence");
mkdirSync(evidenceDir, { recursive: true });
const stamp = new Date().toISOString().replace(/[-:]/g, "").replace(/\..+$/, "");
const outPath =
  process.env.H7_ASSURANCE_OUT ?? path.join(evidenceDir, `h7-assurance-bundle-${stamp}.json`);
...
const payload = {
  ...
  horizon: "H7",
  pass: horizonStatus.pass && auditRotation.pass,
  checks: {
    horizonStatusPass: horizonStatus.pass,
    auditRotationPass: auditRotation.pass,
  },
  steps: [horizonStatus, auditRotation],
};
