#!/usr/bin/env node
/**
 * Records machine-readable evidence for validate:manifest-schemas on the newest post-H27 sustainment loop (H28 closeout).
 * Runs **`npm run validate:manifest-schemas`**, validates the newest **`post-h27-sustainment-loop-*.json`**
 * with **`validateManifestSchema("post-h27-sustainment-loop", …)`**, and writes **`evidence/manifest-schemas-post-h27-loop-evidence-*.json`**.
 */
...
const evidenceDir = process.env.MANIFEST_SCHEMAS_POST_H27_LOOP_EVIDENCE_DIR ?? path.join(root, "evidence");
...
if (sustainmentPath) {
  try {
    ...
    const schema = validateManifestSchema("post-h27-sustainment-loop", j);
...
const outPath =
  process.env.MANIFEST_SCHEMAS_POST_H27_LOOP_EVIDENCE_OUT ??
...
const payload = {
  ...
  checks: {
    ...
    postH27SustainmentLoopSchemaPass: sustainmentSchemaPass,
    postH27SustainmentLoopPayloadPass: sustainmentLoopPass,
  },
  steps: [
    {
