#!/usr/bin/env node
/**
 * Manifest schema pin on the newest post-H28 sustainment loop artifact (H29).
 */
...
const evidenceDir = process.env.MANIFEST_SCHEMAS_POST_H28_EVIDENCE_DIR ?? path.join(root, "evidence");
...
if (loopPath) {
  try {
    ...
    const v = validateManifestSchema("post-h28-sustainment-loop", payload);
    schemaPass = v.valid;
...
const outPath =
  process.env.MANIFEST_SCHEMAS_POST_H28_EVIDENCE_OUT ??
