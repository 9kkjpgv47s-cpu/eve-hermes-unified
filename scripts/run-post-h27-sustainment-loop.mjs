#!/usr/bin/env node
/**
 * Post-H27 sustainment loop: post-H26 chain plus terminal manifest schema evidence and H27 closeout gate.
 *
 * Run **`npm run run:h16-assurance-bundle`** first when reproducing a full chain locally or after **`npm run validate:all`** when **`evidence/`** lacks goal-policy output through **H27**.
 */
...
const manifestSchemasTerminal = runNpm("run:manifest-schemas-terminal-evidence");
...
const manifestSchemasTerminalEvidencePass = manifestSchemasTerminal.exitCode === 0;
...
const manifest = {
  ...
  steps: [
    ...
    { id: "run_manifest_schemas_terminal_evidence", ...manifestSchemasTerminal },
