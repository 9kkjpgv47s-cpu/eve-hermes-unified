#!/usr/bin/env node
/**
 * Post-H30 sustainment loop: horizon metadata, H17/H18 merge and rehearsal gates, CI soak,
 * entrypoints + shell CI + tenant isolation, H30 terminal assurance bundle (H29 + horizon-status),
 * and H30 closeout gate.
 *
 * Run **`npm run run:h16-assurance-bundle`** first when reproducing a full chain locally or after **`npm run validate:all`** when **`evidence/`** lacks goal-policy output through **H30**.
 */
...
const assuranceH30 = runNode("scripts/run-h30-assurance-bundle.mjs");
