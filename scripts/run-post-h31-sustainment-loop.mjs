#!/usr/bin/env node
/**
 * Post-H31 sustainment loop: same terminal chain as post-H30, plus H31 assurance bundle
 * (post-H30 + manifest schemas) and H31 closeout gate.
 *
 * Run **`npm run run:h16-assurance-bundle`** first when reproducing a full chain locally or after **`npm run validate:all`** when **`evidence/`** lacks goal-policy output through **H31**.
 */
...
const evidenceDir = process.env.POST_H31_SUSTAINMENT_EVIDENCE_DIR ?? path.join(root, "evidence");
...
const manifestPath =
  process.env.POST_H31_SUSTAINMENT_LOOP_OUT ??
  path.join(evidenceDir, `post-h31-sustainment-loop-${stamp}.json`);
...
const assuranceH31 = runNode("scripts/run-h31-assurance-bundle.mjs");
const closeout = runNpm("validate:h31-closeout");
...
const h31AssuranceBundlePass = assuranceH31.exitCode === 0;
const h31CloseoutGatePass = closeout.exitCode === 0;
const pass =
  horizonStatusPass &&
...
const manifest = {
  ...
  steps: [
    ...
    { id: "run_h31_assurance_bundle", ...assuranceH31 },
    { id: "validate_h31_closeout", ...closeout },
  ],
};
