#!/usr/bin/env node
/**
 * Post-H32 sustainment loop: horizon metadata, H17/H18 merge and rehearsal gates, CI soak,
 * entrypoints + shell CI + tenant isolation, H32 terminal assurance bundle (H31 + horizon-status),
 * and H32 closeout gate.
 *
 * Run **`npm run run:h16-assurance-bundle`** first when reproducing a full chain locally or after **`npm run validate:all`** when **`evidence/`** lacks goal-policy output through **H32**.
 */
...
const evidenceDir = process.env.POST_H32_SUSTAINMENT_EVIDENCE_DIR ?? path.join(root, "evidence");
...
const manifestPath =
  process.env.POST_H32_SUSTAINMENT_LOOP_OUT ??
  path.join(evidenceDir, `post-h32-sustainment-loop-${stamp}.json`);
...
const assuranceH32 = runNode("scripts/run-h32-assurance-bundle.mjs");
const closeout = runNpm("validate:h32-closeout");
...
const h32AssuranceBundlePass = assuranceH32.exitCode === 0;
const h32CloseoutGatePass = closeout.exitCode === 0;
const pass =
  horizonStatusPass &&
...
const manifest = {
  ...
  steps: [
    ...
    { id: "run_h32_assurance_bundle", ...assuranceH32 },
    { id: "validate_h32_closeout", ...closeout },
  ],
};
