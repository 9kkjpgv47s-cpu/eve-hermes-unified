#!/usr/bin/env node
/**
 * Post-H33 sustainment loop: same terminal chain as post-H32, plus H33 assurance bundle
 * (post-H32 + manifest schemas) and H33 closeout gate.
 *
 * Run **`npm run run:h16-assurance-bundle`** first when reproducing a full chain locally or after **`npm run validate:all`** when **`evidence/`** lacks goal-policy output through **H33**.
 */
...
const evidenceDir = process.env.POST_H33_SUSTAINMENT_EVIDENCE_DIR ?? path.join(root, "evidence");
...
const manifestPath =
  process.env.POST_H33_SUSTAINMENT_LOOP_OUT ??
  path.join(evidenceDir, `post-h33-sustainment-loop-${stamp}.json`);
...
const assuranceH33 = runNode("scripts/run-h33-assurance-bundle.mjs");
const closeout = runNpm("validate:h33-closeout");
...
const h33AssuranceBundlePass = assuranceH33.exitCode === 0;
const h33CloseoutGatePass = closeout.exitCode === 0;
const pass =
  horizonStatusPass &&
...
const manifest = {
  ...
  steps: [
    ...
    { id: "run_h33_assurance_bundle", ...assuranceH33 },
    { id: "validate_h33_closeout", ...closeout },
  ],
};
