#!/usr/bin/env node
/**
 * Post-H34 sustainment loop: horizon metadata, H17/H18 merge and rehearsal gates, CI soak,
 * entrypoints + shell CI + tenant isolation, H34 terminal assurance bundle (H33 + horizon-status),
 * and H34 closeout gate.
 *
 * Run **`npm run run:h16-assurance-bundle`** first when reproducing a full chain locally or after **`npm run validate:all`** when **`evidence/`** lacks goal-policy output through **H34**.
 */
...
const evidenceDir = process.env.POST_H34_SUSTAINMENT_EVIDENCE_DIR ?? path.join(root, "evidence");
...
const manifestPath =
  process.env.POST_H34_SUSTAINMENT_LOOP_OUT ??
  path.join(evidenceDir, `post-h34-sustainment-loop-${stamp}.json`);
...
const assuranceH34 = runNode("scripts/run-h34-assurance-bundle.mjs");
const closeout = runNpm("validate:h34-closeout");
...
const h34AssuranceBundlePass = assuranceH34.exitCode === 0;
const h34CloseoutGatePass = closeout.exitCode === 0;
const pass =
  horizonStatusPass &&
...
const manifest = {
  ...
  steps: [
    ...
    { id: "run_h34_assurance_bundle", ...assuranceH34 },
    { id: "validate_h34_closeout", ...closeout },
  ],
};
