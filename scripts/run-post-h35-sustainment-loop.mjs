#!/usr/bin/env node
/**
 * Post-H35 sustainment loop: same terminal chain as post-H34, plus H35 assurance bundle
 * (post-H34 + manifest schemas) and H35 closeout gate.
 *
 * Run **`npm run run:h16-assurance-bundle`** first when reproducing a full chain locally or after **`npm run validate:all`** when **`evidence/`** lacks goal-policy output through **H35**.
 */
...
const evidenceDir = process.env.POST_H35_SUSTAINMENT_EVIDENCE_DIR ?? path.join(root, "evidence");
...
const manifestPath =
  process.env.POST_H35_SUSTAINMENT_LOOP_OUT ??
  path.join(evidenceDir, `post-h35-sustainment-loop-${stamp}.json`);
...
const assuranceH35 = runNode("scripts/run-h35-assurance-bundle.mjs");
const closeout = runNpm("validate:h35-closeout");
...
const h35AssuranceBundlePass = assuranceH35.exitCode === 0;
const h35CloseoutGatePass = closeout.exitCode === 0;
const pass =
  horizonStatusPass &&
...
const manifest = {
  ...
  steps: [
    ...
    { id: "run_h35_assurance_bundle", ...assuranceH35 },
    { id: "validate_h35_closeout", ...closeout },
  ],
};
