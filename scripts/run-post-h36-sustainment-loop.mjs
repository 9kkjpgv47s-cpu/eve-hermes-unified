#!/usr/bin/env node
/**
 * Post-H36 sustainment loop: horizon metadata, H17/H18 merge and rehearsal gates, CI soak,
 * entrypoints + shell CI + tenant isolation, H36 terminal assurance bundle (H35 + horizon-status),
 * and H36 closeout gate.
 *
 * Run **`npm run run:h16-assurance-bundle`** first when reproducing a full chain locally or after **`npm run validate:all`** when **`evidence/`** lacks goal-policy output through **H36**.
 */
...
const evidenceDir = process.env.POST_H36_SUSTAINMENT_EVIDENCE_DIR ?? path.join(root, "evidence");
...
const manifestPath =
  process.env.POST_H36_SUSTAINMENT_LOOP_OUT ??
  path.join(evidenceDir, `post-h36-sustainment-loop-${stamp}.json`);
...
const assuranceH36 = runNode("scripts/run-h36-assurance-bundle.mjs");
const closeout = runNpm("validate:h36-closeout");
...
const h36AssuranceBundlePass = assuranceH36.exitCode === 0;
const h36CloseoutGatePass = closeout.exitCode === 0;
const pass =
  horizonStatusPass &&
...
const manifest = {
  ...
  steps: [
    ...
    { id: "run_h36_assurance_bundle", ...assuranceH36 },
    { id: "validate_h36_closeout", ...closeout },
  ],
};
