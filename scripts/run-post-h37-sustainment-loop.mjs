#!/usr/bin/env node
/**
 * Post-H37 sustainment loop: same terminal chain as post-H36, plus H37 assurance bundle
 * (post-H36 + manifest schemas) and H37 closeout gate.
 *
 * Run **`npm run run:h16-assurance-bundle`** first when reproducing a full chain locally or after **`npm run validate:all`** when **`evidence/`** lacks goal-policy output through **H37**.
 */
...
const evidenceDir = process.env.POST_H37_SUSTAINMENT_EVIDENCE_DIR ?? path.join(root, "evidence");
...
const manifestPath =
  process.env.POST_H37_SUSTAINMENT_LOOP_OUT ??
  path.join(evidenceDir, `post-h37-sustainment-loop-${stamp}.json`);
...
const assuranceH37 = runNode("scripts/run-h37-assurance-bundle.mjs");
const closeout = runNpm("validate:h37-closeout");
...
const h37AssuranceBundlePass = assuranceH37.exitCode === 0;
const h37CloseoutGatePass = closeout.exitCode === 0;
const pass =
  horizonStatusPass &&
...
const manifest = {
  ...
  steps: [
    ...
    { id: "run_h37_assurance_bundle", ...assuranceH37 },
    { id: "validate_h37_closeout", ...closeout },
  ],
};
