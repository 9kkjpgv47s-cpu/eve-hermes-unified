#!/usr/bin/env node
/**
 * Post-H38 sustainment loop: horizon metadata, H17/H18 merge and rehearsal gates, CI soak,
 * entrypoints + shell CI + tenant isolation, H38 terminal assurance bundle (H37 + horizon-status),
 * and H38 closeout gate.
 *
 * Run **`npm run run:h16-assurance-bundle`** first when reproducing a full chain locally or after **`npm run validate:all`** when **`evidence/`** lacks goal-policy output through **H38**.
 */
...
const evidenceDir = process.env.POST_H38_SUSTAINMENT_EVIDENCE_DIR ?? path.join(root, "evidence");
...
const manifestPath =
  process.env.POST_H38_SUSTAINMENT_LOOP_OUT ??
  path.join(evidenceDir, `post-h38-sustainment-loop-${stamp}.json`);
...
const assuranceH38 = runNode("scripts/run-h38-assurance-bundle.mjs");
const closeout = runNpm("validate:h38-closeout");
...
const h38AssuranceBundlePass = assuranceH38.exitCode === 0;
const h38CloseoutGatePass = closeout.exitCode === 0;
const pass =
  horizonStatusPass &&
...
const manifest = {
  ...
  steps: [
    ...
    { id: "run_h38_assurance_bundle", ...assuranceH38 },
    { id: "validate_h38_closeout", ...closeout },
  ],
};
