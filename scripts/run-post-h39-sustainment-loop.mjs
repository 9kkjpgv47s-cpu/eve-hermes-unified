#!/usr/bin/env node
/**
 * Post-H39 sustainment loop: same terminal chain as post-H38, plus H39 assurance bundle
 * (post-H38 + manifest schemas) and H39 closeout gate.
 *
 * Run **`npm run run:h16-assurance-bundle`** first when reproducing a full chain locally or after **`npm run validate:all`** when **`evidence/`** lacks goal-policy output through **H39**.
 */
...
const evidenceDir = process.env.POST_H39_SUSTAINMENT_EVIDENCE_DIR ?? path.join(root, "evidence");
...
const manifestPath =
  process.env.POST_H39_SUSTAINMENT_LOOP_OUT ??
  path.join(evidenceDir, `post-h39-sustainment-loop-${stamp}.json`);
...
const assuranceH39 = runNode("scripts/run-h39-assurance-bundle.mjs");
const closeout = runNpm("validate:h39-closeout");
...
const h39AssuranceBundlePass = assuranceH39.exitCode === 0;
const h39CloseoutGatePass = closeout.exitCode === 0;
const pass =
  horizonStatusPass &&
...
const manifest = {
  ...
  steps: [
    ...
    { id: "run_h39_assurance_bundle", ...assuranceH39 },
    { id: "validate_h39_closeout", ...closeout },
  ],
};
