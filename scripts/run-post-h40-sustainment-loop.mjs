#!/usr/bin/env node
/**
 * Post-H40 sustainment loop: horizon metadata, H17/H18 merge and rehearsal gates, CI soak,
 * entrypoints + shell CI + tenant isolation, H40 terminal assurance bundle (H39 + horizon-status),
 * and H40 closeout gate.
 *
 * Run **`npm run run:h16-assurance-bundle`** first when reproducing a full chain locally or after **`npm run validate:all`** when **`evidence/`** lacks goal-policy output through **H40**.
 */
...
const evidenceDir = process.env.POST_H40_SUSTAINMENT_EVIDENCE_DIR ?? path.join(root, "evidence");
...
const manifestPath =
  process.env.POST_H40_SUSTAINMENT_LOOP_OUT ??
  path.join(evidenceDir, `post-h40-sustainment-loop-${stamp}.json`);
...
const assuranceH40 = runNode("scripts/run-h40-assurance-bundle.mjs");
const closeout = runNpm("validate:h40-closeout");
...
const h40AssuranceBundlePass = assuranceH40.exitCode === 0;
const h40CloseoutGatePass = closeout.exitCode === 0;
const pass =
  horizonStatusPass &&
...
const manifest = {
  ...
  steps: [
    ...
    { id: "run_h40_assurance_bundle", ...assuranceH40 },
    { id: "validate_h40_closeout", ...closeout },
  ],
};
