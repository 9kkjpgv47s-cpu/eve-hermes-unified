#!/usr/bin/env node
/**
 * Post-H41 sustainment loop: horizon metadata, H17/H18 merge and rehearsal gates, CI soak,
 * entrypoints + shell CI + tenant isolation, H41 terminal assurance bundle (post-H40 + manifest schemas),
 * and H41 closeout gate.
 *
 * Run **`npm run run:h16-assurance-bundle`** first when reproducing a full chain locally or after **`npm run validate:all`** when **`evidence/`** lacks goal-policy output through **H42**.
 */
...
const evidenceDir = process.env.POST_H41_SUSTAINMENT_EVIDENCE_DIR ?? path.join(root, "evidence");
...
const manifestPath =
  process.env.POST_H41_SUSTAINMENT_LOOP_OUT ??
  path.join(evidenceDir, `post-h41-sustainment-loop-${stamp}.json`);
...
const assuranceH41 = runNode("scripts/run-h41-assurance-bundle.mjs");
const closeout = runNpm("validate:h41-closeout");
...
const h41AssuranceBundlePass = assuranceH41.exitCode === 0;
const h41CloseoutGatePass = closeout.exitCode === 0;
const pass =
  horizonStatusPass &&
...
const manifest = {
  ...
  steps: [
    ...
    { id: "run_h41_assurance_bundle", ...assuranceH41 },
    { id: "validate_h41_closeout", ...closeout },
  ],
};
