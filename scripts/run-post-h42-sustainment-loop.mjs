#!/usr/bin/env node
/**
 * Post-H42 sustainment loop: horizon metadata, H17/H18 merge and rehearsal gates, CI soak,
 * entrypoints + shell CI + tenant isolation, H42 terminal assurance bundle (H41 + horizon-status),
 * and H42 closeout gate.
 *
...
const evidenceDir = process.env.POST_H42_SUSTAINMENT_EVIDENCE_DIR ?? path.join(root, "evidence");
...
const manifestPath =
  process.env.POST_H42_SUSTAINMENT_LOOP_OUT ??
  path.join(evidenceDir, `post-h42-sustainment-loop-${stamp}.json`);
...
const assuranceH42 = runNode("scripts/run-h42-assurance-bundle.mjs");
const closeout = runNpm("validate:h42-closeout");
...
const h42AssuranceBundlePass = assuranceH42.exitCode === 0;
const h42CloseoutGatePass = closeout.exitCode === 0;
const pass =
  horizonStatusPass &&
...
const manifest = {
  ...
  steps: [
    ...
    { id: "run_h42_assurance_bundle", ...assuranceH42 },
    { id: "validate_h42_closeout", ...closeout },
  ],
};
