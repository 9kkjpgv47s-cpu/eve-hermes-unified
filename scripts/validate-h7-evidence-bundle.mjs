#!/usr/bin/env node
/**
 * H7 evidence bundle gate (h7-action-3): same machine checks as validate-h5-evidence-bundle
 * (soak tenant/region/partition dimensions, region drill, partition drill, rollback, remediation).
 * Emits `h7-closeout-evidence-*.json` (horizon-closeout schema) so `validate-h8-closeout` can wrap
 * the latest passing manifest for promote:horizon H7→H8 with `--goal-policy-key H7->H8`.
 */
...
async function main() {
  ...
  const outPath =
    opts.out ||
    path.join(evidenceDir, `h7-closeout-evidence-${new Date().toISOString().replace(/[:.]/g, "-")}.json`);
  ...
  if (!manifest.pass) {
    process.stderr.write(`H7 evidence bundle validation failed:\n- ${manifest.failures.join("\n- ")}\n`);
    process.exitCode = 2;
  }
}
