#!/usr/bin/env node
/**
 * H8 evidence bundle gate (h8-action-3): same machine checks as validate-h5-evidence-bundle
 * (soak tenant/region/partition dimensions, region drill, partition drill, rollback, remediation).
 * Emits `h8-closeout-evidence-*.json` (horizon-closeout schema) so `validate-h9-closeout` can wrap
 * the latest passing manifest for promote:horizon H8→H9 with `--goal-policy-key H8->H9`.
 */
...
async function main() {
  ...
  const outPath =
    opts.out ||
    path.join(evidenceDir, `h8-closeout-evidence-${new Date().toISOString().replace(/[:.]/g, "-")}.json`);
  ...
  if (!manifest.pass) {
    process.stderr.write(`H8 evidence bundle validation failed:\n- ${manifest.failures.join("\n- ")}\n`);
    process.exitCode = 2;
  }
}
