#!/usr/bin/env node
/**
 * H9 evidence bundle gate (h9-action-3): same scale checks as validate-h5-evidence-bundle.
 * Emits `h9-closeout-evidence-*.json` (horizon-closeout schema) so `validate-h10-closeout` can wrap
 * the latest passing manifest for promote:horizon H9→H10 with `--goal-policy-key H9->H10`.
 */
...
async function main() {
  ...
  const outPath =
    opts.out ||
    path.join(evidenceDir, `h9-closeout-evidence-${new Date().toISOString().replace(/[:.]/g, "-")}.json`);
  ...
  if (!manifest.pass) {
    process.stderr.write(`H9 evidence bundle validation failed:\n- ${manifest.failures.join("\n- ")}\n`);
    process.exitCode = 2;
  }
}
