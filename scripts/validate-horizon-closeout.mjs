...
function commandVerificationType(command) {
  ...
  if (command === "node ./scripts/run-evidence-summary-evidence.mjs") {
    return "evidence-summary-evidence";
  }
  if (command === "node ./scripts/run-evidence-gates-evidence.mjs") {
    return "evidence-gates-evidence";
  }
  if (command === "node ./scripts/run-horizon-status-evidence.mjs") {
    return "horizon-status-evidence";
  }
...
function evaluateCommandPayload(command, payload, targetHorizon = "") {
  ...
  if (verificationType === "failure-injection-evidence") {
    const checks = [];
    if (payload.pass !== true) {
      checks.push("failure_injection_evidence_not_passed");
    }
    const signal = payload.checks && typeof payload.checks === "object" ? payload.checks : {};
    if (signal.failureInjectionSmokePass !== true) {
      checks.push("failure_injection_smoke_not_passed");
    }
    return { pass: checks.length === 0, checks };
  }
  ...
  if (verificationType === "evidence-summary-evidence") {
    const checks = [];
    if (payload.pass !== true) {
      checks.push("evidence_summary_evidence_not_passed");
    }
    const signal = payload.checks && typeof payload.checks === "object" ? payload.checks : {};
    if (signal.evidenceSummaryGatePass !== true) {
      checks.push("evidence_summary_gate_not_passed");
    }
    return { pass: checks.length === 0, checks };
  }
  if (verificationType === "evidence-gates-evidence") {
    ...
    const signal = payload.checks && typeof payload.checks === "object" ? payload.checks : {};
    if (signal.evidenceGatesPass !== true) {
      checks.push("evidence_gates_not_passed");
    }
    return { pass: checks.length === 0, checks };
  }
  if (verificationType === "horizon-status-evidence") {
    const checks = [];
    if (payload.pass !== true) {
      checks.push("horizon_status_evidence_not_passed");
    }
    const signal = payload.checks && typeof payload.checks === "object" ? payload.checks : {};
    if (signal.horizonStatusValidatorPass !== true) {
      checks.push("horizon_status_validator_not_passed");
    }
    if (signal.horizonStatusReportedValid !== true) {
      checks.push("horizon_status_reported_invalid");
    }
    return { pass: checks.length === 0, checks };
  }
  ...
  if (verificationType === "post-h23-sustainment-loop") {
    ...
    if (signal.postH22SustainmentLoopPass !== true) {
      checks.push("post_h23_post_h22_sustainment_loop_not_passed");
    }
    if (signal.regionFailoverEvidencePass !== true) {
      checks.push("post_h23_region_failover_evidence_not_passed");
    }
  ...
  if (verificationType === "post-h25-sustainment-loop") {
    ...
    if (signal.postH24SustainmentLoopPass !== true) {
      checks.push("post_h25_post_h24_sustainment_loop_not_passed");
    }
    if (signal.emergencyRollbackEvidencePass !== true) {
      checks.push("post_h25_emergency_rollback_evidence_not_passed");
    }
  ...
  if (verificationType === "post-h26-sustainment-loop") {
    ...
    const signal = payload.checks && typeof payload.checks === "object" ? payload.checks : {};
    if (signal.postH25SustainmentLoopPass !== true) {
      checks.push("post_h26_post_h25_sustainment_loop_not_passed");
    }
    if (signal.failureInjectionEvidencePass !== true) {
      checks.push("post_h26_failure_injection_evidence_not_passed");
    }
  ...
  if (verificationType === "post-h28-sustainment-loop") {
    ...
    const signal = payload.checks && typeof payload.checks === "object" ? payload.checks : {};
    if (signal.postH27SustainmentLoopPass !== true) {
      checks.push("post_h28_post_h27_sustainment_loop_not_passed");
    }
    if (signal.evidenceSummaryEvidencePass !== true) {
      checks.push("post_h28_evidence_summary_evidence_not_passed");
    }
  ...
  if (verificationType === "post-h29-sustainment-loop") {
    ...
    if (signal.postH28SustainmentLoopPass !== true) {
      checks.push("post_h29_post_h28_sustainment_loop_not_passed");
    }
    if (signal.evidenceGatesEvidencePass !== true) {
      checks.push("post_h29_evidence_gates_evidence_not_passed");
    }
  ...
  if (verificationType === "post-h30-sustainment-loop") {
    ...
    const signal = payload.checks && typeof payload.checks === "object" ? payload.checks : {};
    if (signal.postH29SustainmentLoopPass !== true) {
      checks.push("post_h30_post_h29_sustainment_loop_not_passed");
    }
    if (signal.horizonStatusEvidencePass !== true) {
      checks.push("post_h30_horizon_status_evidence_not_passed");
    }
    ...
    const requiredEvidence = Array.isArray(horizonStatus.requiredEvidence)
      ? horizonStatus.requiredEvidence.filter(
          (entry) =>
            ...
            !(
              targetHorizon === "H25" &&
              (String(entry.id ?? "") === "h25-post-sustainment-loop" ||
                String(entry.command ?? "").includes("run-post-h25-sustainment-loop"))
            ) &&
            !(
              targetHorizon === "H26" &&
            ...
            !(
              targetHorizon === "H28" &&
              (String(entry.id ?? "") === "h28-post-sustainment-loop" ||
                String(entry.command ?? "").includes("run-post-h28-sustainment-loop"))
            ) &&
            !(
              targetHorizon === "H29" &&
              (String(entry.id ?? "") === "h29-post-sustainment-loop" ||
                String(entry.command ?? "").includes("run-post-h29-sustainment-loop"))
            ) &&
            !(
              targetHorizon === "H30" &&
              (String(entry.id ?? "") === "h30-post-sustainment-loop" ||
                String(entry.command ?? "").includes("run-post-h30-sustainment-loop"))
            ),
        )
      : [];
