#!/usr/bin/env node
/**
 * Validates the newest evidence/post-h16-sustainment-loop-*.json (after npm run verify:sustainment-loop).
 */
...
const evidenceDir = path.resolve(process.cwd(), process.env.POST_H16_SUSTAINMENT_EVIDENCE_DIR ?? "evidence");


const manifestPath = await newestMatchingEvidenceFile(evidenceDir, "post-h16-sustainment-loop-");
if (!manifestPath) {
  process.stderr.write(`No evidence/post-h16-sustainment-loop-*.json under ${evidenceDir}\n`);
  process.exit(2);
}
