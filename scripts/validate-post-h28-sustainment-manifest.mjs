#!/usr/bin/env node
/**
 * Validates the newest evidence/post-h28-sustainment-loop-*.json (after npm run verify:sustainment-loop).
 */
...
async function newestLoopManifest(dir) {
  ...
  const files = entries
    .filter((e) => e.isFile() && e.name.startsWith("post-h28-sustainment-loop-") && e.name.endsWith(".json"))
...
if (checks.postH27SustainmentChainPass !== true) failures.push("postH27SustainmentChainPass");
if (checks.manifestSchemasPostH27LoopEvidencePass !== true) failures.push("manifestSchemasPostH27LoopEvidencePass");
if (checks.stagePromotionSustainmentEvidencePass !== true) failures.push("stagePromotionSustainmentEvidencePass");
