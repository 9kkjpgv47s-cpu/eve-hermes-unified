...
async function newestLoopManifest(dir) {
  ...
  const files = entries
    .filter((e) => e.isFile() && e.name.startsWith("post-h30-sustainment-loop-") && e.name.endsWith(".json"))
...
if (checks.postH29SustainmentChainPass !== true) failures.push("postH29SustainmentChainPass");
if (checks.manifestSchemasPostH29LoopEvidencePass !== true) failures.push("manifestSchemasPostH29LoopEvidencePass");
