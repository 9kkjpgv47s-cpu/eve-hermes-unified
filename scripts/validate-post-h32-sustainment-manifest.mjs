#!/usr/bin/env node
/**
 * Validates the newest evidence/post-h32-sustainment-loop-*.json (after npm run verify:sustainment-loop).
 */
import { readdir, readFile } from "node:fs/promises";
import path from "node:path";
import process from "node:process";

const evidenceDir = path.resolve(process.cwd(), process.env.POST_H32_SUSTAINMENT_EVIDENCE_DIR ?? "evidence");

async function newestLoopManifest(dir) {
  const entries = await readdir(dir, { withFileTypes: true });
  const files = entries
    .filter((e) => e.isFile() && e.name.startsWith("post-h32-sustainment-loop-") && e.name.endsWith(".json"))
...
if (!manifestPath) {
  process.stderr.write(`No evidence/post-h32-sustainment-loop-*.json under ${evidenceDir}\n`);
  process.exit(2);
}
...
if (checks.h32AssuranceBundlePass !== true) failures.push("h32AssuranceBundlePass");
if (checks.h32CloseoutGatePass !== true) failures.push("h32CloseoutGatePass");

if (failures.length > 0) {
  process.stderr.write(`Invalid post-H32 sustainment manifest ${manifestPath}: ${failures.join(", ")}\n`);
  process.exit(1);
}
