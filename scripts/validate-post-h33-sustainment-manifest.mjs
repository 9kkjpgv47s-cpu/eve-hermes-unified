#!/usr/bin/env node
/**
 * Validates the newest evidence/post-h33-sustainment-loop-*.json (after npm run verify:sustainment-loop:h33-legacy).
 */
import { readdir, readFile } from "node:fs/promises";
import path from "node:path";
import process from "node:process";

const evidenceDir = path.resolve(process.cwd(), process.env.POST_H33_SUSTAINMENT_EVIDENCE_DIR ?? "evidence");

async function newestLoopManifest(dir) {
  const entries = await readdir(dir, { withFileTypes: true });
  const files = entries
    .filter((e) => e.isFile() && e.name.startsWith("post-h33-sustainment-loop-") && e.name.endsWith(".json"))
...
if (!manifestPath) {
  process.stderr.write(`No evidence/post-h33-sustainment-loop-*.json under ${evidenceDir}\n`);
  process.exit(2);
}
...
if (checks.h33AssuranceBundlePass !== true) failures.push("h33AssuranceBundlePass");
if (checks.h33CloseoutGatePass !== true) failures.push("h33CloseoutGatePass");

if (failures.length > 0) {
  process.stderr.write(`Invalid post-H33 sustainment manifest ${manifestPath}: ${failures.join(", ")}\n`);
  process.exit(1);
}
