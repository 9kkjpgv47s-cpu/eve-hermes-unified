#!/usr/bin/env node
/**
 * Validates the newest evidence/post-h34-sustainment-loop-*.json (after npm run verify:sustainment-loop).
 */
import { readdir, readFile } from "node:fs/promises";
import path from "node:path";
import process from "node:process";

const evidenceDir = path.resolve(process.cwd(), process.env.POST_H34_SUSTAINMENT_EVIDENCE_DIR ?? "evidence");

async function newestLoopManifest(dir) {
  const entries = await readdir(dir, { withFileTypes: true });
  const files = entries
    .filter((e) => e.isFile() && e.name.startsWith("post-h34-sustainment-loop-") && e.name.endsWith(".json"))
...
if (!manifestPath) {
  process.stderr.write(`No evidence/post-h34-sustainment-loop-*.json under ${evidenceDir}\n`);
  process.exit(2);
}
...
if (checks.h34AssuranceBundlePass !== true) failures.push("h34AssuranceBundlePass");
if (checks.h34CloseoutGatePass !== true) failures.push("h34CloseoutGatePass");

if (failures.length > 0) {
  process.stderr.write(`Invalid post-H34 sustainment manifest ${manifestPath}: ${failures.join(", ")}\n`);
  process.exit(1);
}
