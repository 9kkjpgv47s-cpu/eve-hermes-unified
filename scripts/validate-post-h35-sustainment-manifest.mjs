#!/usr/bin/env node
/**
 * Validates the newest evidence/post-h35-sustainment-loop-*.json (after npm run verify:sustainment-loop:h35-legacy).
 */
import { readdir, readFile } from "node:fs/promises";
import path from "node:path";
import process from "node:process";

const evidenceDir = path.resolve(process.cwd(), process.env.POST_H35_SUSTAINMENT_EVIDENCE_DIR ?? "evidence");

async function newestLoopManifest(dir) {
  const entries = await readdir(dir, { withFileTypes: true });
  const files = entries
    .filter((e) => e.isFile() && e.name.startsWith("post-h35-sustainment-loop-") && e.name.endsWith(".json"))
...
if (!manifestPath) {
  process.stderr.write(`No evidence/post-h35-sustainment-loop-*.json under ${evidenceDir}\n`);
  process.exit(2);
}
...
if (checks.h35AssuranceBundlePass !== true) failures.push("h35AssuranceBundlePass");
if (checks.h35CloseoutGatePass !== true) failures.push("h35CloseoutGatePass");

if (failures.length > 0) {
  process.stderr.write(`Invalid post-H35 sustainment manifest ${manifestPath}: ${failures.join(", ")}\n`);
  process.exit(1);
}
