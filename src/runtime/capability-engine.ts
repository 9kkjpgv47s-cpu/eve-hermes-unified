...
function raceExecutorWithTimeout<T>(promise: Promise<T>, timeoutMs: number, onTimeout: () => T): Promise<T> {
...
export class UnifiedCapabilityEngine implements CapabilityEngine {
  ...
  async execute(
    ...
    const timeoutResult = (): RegistryCapabilityExecutionResult => ({
      consumed: false,
      responseText: `Capability '${selection.id}' exceeded execution timeout of ${timeoutMs}ms.`,
      reason: "capability_execution_timeout",
      failureClass: "dispatch_failure",
      metadata: { capabilityId: selection.id, timeoutMs: String(timeoutMs) },
    });
    const execution = await raceExecutorWithTimeout(executionPromise, timeoutMs, timeoutResult);
