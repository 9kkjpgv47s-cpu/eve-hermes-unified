...
async function seedHorizonStatus(
  ...
  const withH5H6 = options?.withH5H6 ?? false;
  ...
  const withH10H11 = options?.withH10H11 ?? false;
  const withH11H12 = options?.withH11H12 ?? false;
  ...
  if (withH6H7) {
    transitions["H6->H7"] = {
      ...
      requiredTaggedActionCounts: includeTaggedCounts
        ? {
            validation: { minCount: 1, minPendingCount: 1 },
            durability: { minCount: 1, minPendingCount: 1 },
          }
        : undefined,
    };
  }
  ...
  if (withH5H6) {
    nextActions.push(
      { id: "h6-action-1", summary: "seed", targetHorizon: "H6", status: "planned", tags: ["validation"] },
      { id: "h6-action-2", summary: "seed", targetHorizon: "H6", status: "planned", tags: ["operations"] },
    );
  }
  if (withH6H7) {
    nextActions.push(
      { id: "h7-action-1", summary: "seed", targetHorizon: "H7", status: "planned", tags: ["validation"] },
      { id: "h7-action-2", summary: "seed", targetHorizon: "H7", status: "planned", tags: ["durability"] },
    );
  }
  if (withH7H8) {
    nextActions.push(
      { id: "h8-action-1", summary: "seed", targetHorizon: "H8", status: "planned", tags: ["validation"] },
      { id: "h8-action-2", summary: "seed", targetHorizon: "H8", status: "planned", tags: ["operations"] },
    );
  }
  if (withH8H9) {
    nextActions.push(
  ...
  if (withH10H11) {
    nextActions.push(
      { id: "h11-action-1", summary: "seed", targetHorizon: "H11", status: "planned", tags: ["validation"] },
      { id: "h11-action-2", summary: "seed", targetHorizon: "H11", status: "planned", tags: ["durability"] },
    );
  }
  if (withH11H12) {
    nextActions.push(
      { id: "h12-action-1", summary: "seed", targetHorizon: "H12", status: "planned", tags: ["validation"] },
      { id: "h12-action-2", summary: "seed", targetHorizon: "H12", status: "planned", tags: ["operations"] },
    );
  }
...
async function seedGoalPolicyFile(
  ...
  options?: {
    ...
    includeH10H11?: boolean;
    includeH11H12?: boolean;
    includeTaggedCounts?: boolean;
  },
): Promise<void> {
  const includeH3H4 = options?.includeH3H4 ?? true;
  const includeH4H5 = options?.includeH4H5 ?? true;
  const includeH5H6 = options?.includeH5H6 ?? false;
  ...
  const includeH10H11 = options?.includeH10H11 ?? false;
  const includeH11H12 = options?.includeH11H12 ?? false;
  const includeTaggedCounts = options?.includeTaggedCounts ?? true;
  const transitions: Record<string, unknown> = {
    "H2->H3": {
...
describe("check-goal-policy-coverage.mjs", () => {
  ...
  it("passes when H5->H6 transition is included in coverage window", async () => {
    await withTempDir(async (dir) => {
      const statusPath = path.join(dir, "HORIZON_STATUS.json");
      const outPath = path.join(dir, "goal-policy-coverage-h6.json");
      ...
      expect(payload.pass).toBe(true);
      expect(payload.checks.transitionCount).toBe(4);
      expect(payload.checks.transitionKeys).toEqual(["H2->H3", "H3->H4", "H4->H5", "H5->H6"]);
    });
  });

  it("passes when H6->H7 transition is included in coverage window", async () => {
    await withTempDir(async (dir) => {
      const statusPath = path.join(dir, "HORIZON_STATUS.json");
      const outPath = path.join(dir, "goal-policy-coverage-h7.json");
      await seedHorizonStatus(statusPath, {
        withH3H4: true,
        withH4H5: true,
        withH5H6: true,
      ...
      expect(payload.pass).toBe(true);
      expect(payload.checks.transitionCount).toBe(5);
      expect(payload.checks.transitionKeys).toEqual(["H2->H3", "H3->H4", "H4->H5", "H5->H6", "H6->H7"]);
    });
  });

  it("passes when H7->H8 transition is included in coverage window", async () => {
    await withTempDir(async (dir) => {
      const statusPath = path.join(dir, "HORIZON_STATUS.json");
      const outPath = path.join(dir, "goal-policy-coverage-h8.json");
      await seedHorizonStatus(statusPath, {
        withH3H4: true,
        withH4H5: true,
        withH5H6: true,
      ...
      expect(payload.pass).toBe(true);
      expect(payload.checks.transitionCount).toBe(6);
  ...
  it("passes when H8->H9 transition is included in coverage window", async () => {
    await withTempDir(async (dir) => {
      const statusPath = path.join(dir, "HORIZON_STATUS.json");
      const outPath = path.join(dir, "goal-policy-coverage-h9.json");
      await seedHorizonStatus(statusPath, {
        withH3H4: true,
        withH4H5: true,
        withH5H6: true,
      ...
      expect(payload.pass).toBe(true);
      expect(payload.checks.transitionCount).toBe(7);
      expect(payload.checks.transitionKeys).toEqual([
        "H2->H3",
        "H3->H4",
        "H4->H5",
  ...
  it("passes when H10->H11 transition is included in coverage window", async () => {
    await withTempDir(async (dir) => {
      const statusPath = path.join(dir, "HORIZON_STATUS.json");
      const outPath = path.join(dir, "goal-policy-coverage-h11.json");
      await seedHorizonStatus(statusPath, {
        withH3H4: true,
        withH4H5: true,
        withH5H6: true,
      ...
      expect(payload.pass).toBe(true);
      expect(payload.checks.transitionCount).toBe(9);
      expect(payload.checks.transitionKeys).toEqual([
        "H2->H3",
        "H3->H4",
        "H4->H5",
  ...
  it("passes when H11->H12 transition is included in coverage window", async () => {
    await withTempDir(async (dir) => {
      const statusPath = path.join(dir, "HORIZON_STATUS.json");
      const outPath = path.join(dir, "goal-policy-coverage-h12.json");
      await seedHorizonStatus(statusPath, {
        withH3H4: true,
        withH4H5: true,
        withH5H6: true,
      ...
      expect(payload.pass).toBe(true);
      expect(payload.checks.transitionCount).toBe(10);
      expect(payload.checks.transitionKeys).toEqual([
        "H2->H3",
        "H3->H4",
        "H4->H5",
