...
describe("run-post-h22-legacy-terminal-sustainment-loop.mjs", () => {
  it("exposes verify:sustainment-loop:h22-legacy-terminal npm script", async () => {
    const pkgRaw = await readFile(path.join(repoRoot, "package.json"), "utf8");
    const pkg = JSON.parse(pkgRaw) as { scripts?: Record<string, string> };
    expect(pkg.scripts?.["verify:sustainment-loop:h22-legacy-terminal"]).toContain(
      "run-post-h22-legacy-terminal-sustainment-loop.mjs",
    );
  });

  it(
    "emits pass and structured checks including h22CloseoutGatePass",
    async () => {
      await seedMergeBundleInputs();
      const result = await runCommandWithTimeout(
        ["node", path.join(repoRoot, "scripts/run-post-h22-legacy-terminal-sustainment-loop.mjs")],
        {
          timeoutMs: 540_000,
