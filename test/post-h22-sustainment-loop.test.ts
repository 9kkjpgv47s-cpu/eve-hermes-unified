...
describe("sustainment loop npm scripts", () => {
  it("maps verify:sustainment-loop to post-H30 terminal chain", async () => {
    const pkgRaw = await readFile(path.join(repoRoot, "package.json"), "utf8");
    const pkg = JSON.parse(pkgRaw) as { scripts?: Record<string, string> };
    expect(pkg.scripts?.["verify:sustainment-loop"]).toContain("run-post-h30-sustainment-loop.mjs");
  });

  it("exposes verify:sustainment-loop:h29-legacy for the prior post-H29-only chain", async () => {
    const pkgRaw = await readFile(path.join(repoRoot, "package.json"), "utf8");
    const pkg = JSON.parse(pkgRaw) as { scripts?: Record<string, string> };
    expect(pkg.scripts?.["verify:sustainment-loop:h29-legacy"]).toContain("run-post-h29-sustainment-loop.mjs");
  });

  it("exposes verify:sustainment-loop:h28-legacy for the prior post-H28-only chain", async () => {
    const pkgRaw = await readFile(path.join(repoRoot, "package.json"), "utf8");
    const pkg = JSON.parse(pkgRaw) as { scripts?: Record<string, string> };
    expect(pkg.scripts?.["verify:sustainment-loop:h28-legacy"]).toContain("run-post-h28-sustainment-loop.mjs");
  });

  it("exposes verify:sustainment-loop:h27-legacy for the prior post-H27-only chain", async () => {
    const pkgRaw = await readFile(path.join(repoRoot, "package.json"), "utf8");
    const pkg = JSON.parse(pkgRaw) as { scripts?: Record<string, string> };
    expect(pkg.scripts?.["verify:sustainment-loop:h27-legacy"]).toContain("run-post-h27-sustainment-loop.mjs");
  });
...
describe("run-post-h28-sustainment-loop.mjs", () => {
  it(
    "emits pass and structured checks (post-H27 chain + evidence-summary + H28 closeout gate)",
    async () => {
      await seedMergeBundleInputs();
      const result = await runCommandWithTimeout(
      ...
      const payload = JSON.parse(raw) as {
        pass?: boolean;
        checks?: {
          postH27SustainmentLoopPass?: boolean;
          evidenceSummaryEvidencePass?: boolean;
          h28CloseoutGatePass?: boolean;
        };
      };
      expect(payload.pass).toBe(true);
      expect(payload.checks?.postH27SustainmentLoopPass).toBe(true);
      expect(payload.checks?.evidenceSummaryEvidencePass).toBe(true);
...
describe("run-post-h29-sustainment-loop.mjs", () => {
  it(
    "emits pass and structured checks (post-H28 chain + evidence-gates + H29 closeout gate)",
    async () => {
      await seedMergeBundleInputs();
      const result = await runCommandWithTimeout(
        ["node", path.join(repoRoot, "scripts/run-post-h29-sustainment-loop.mjs")],
        {
          timeoutMs: 960_000,
          env: mergeEnv({ UNIFIED_CI_SOAK_ITERATIONS: "15" }),
        },
      ...
      const payload = JSON.parse(raw) as {
        pass?: boolean;
        checks?: {
          postH28SustainmentLoopPass?: boolean;
          evidenceGatesEvidencePass?: boolean;
          h29CloseoutGatePass?: boolean;
        };
      };
      expect(payload.pass).toBe(true);
      expect(payload.checks?.postH28SustainmentLoopPass).toBe(true);
      expect(payload.checks?.evidenceGatesEvidencePass).toBe(true);
      expect(payload.checks?.h29CloseoutGatePass).toBe(true);
    },
    1_200_000,
  );

  it("validate:post-h29-sustainment-manifest passes on latest loop output", async () => {
    const result = await runCommandWithTimeout(
      ["node", path.join(repoRoot, "scripts/validate-post-h29-sustainment-manifest.mjs")],
      { timeoutMs: 15_000 },
    );
    expect(result.code).toBe(0);
  });
...
describe("run-post-h30-sustainment-loop.mjs", () => {
  it(
    "emits pass and structured checks (post-H29 chain + horizon-status + H30 closeout gate)",
    async () => {
      await seedMergeBundleInputs();
      const result = await runCommandWithTimeout(
        ["node", path.join(repoRoot, "scripts/run-post-h30-sustainment-loop.mjs")],
        {
          timeoutMs: 1_320_000,
          env: mergeEnv({ UNIFIED_CI_SOAK_ITERATIONS: "15" }),
        },
      ...
      const payload = JSON.parse(raw) as {
        pass?: boolean;
        checks?: {
          postH29SustainmentLoopPass?: boolean;
          horizonStatusEvidencePass?: boolean;
          h30CloseoutGatePass?: boolean;
        };
      };
      expect(payload.pass).toBe(true);
      expect(payload.checks?.postH29SustainmentLoopPass).toBe(true);
      expect(payload.checks?.horizonStatusEvidencePass).toBe(true);
      expect(payload.checks?.h30CloseoutGatePass).toBe(true);
    },
    1_500_000,
  );

  it("validate:post-h30-sustainment-manifest passes on latest loop output", async () => {
    const result = await runCommandWithTimeout(
      ["node", path.join(repoRoot, "scripts/validate-post-h30-sustainment-manifest.mjs")],
      { timeoutMs: 15_000 },
    );
    expect(result.code).toBe(0);
  });
