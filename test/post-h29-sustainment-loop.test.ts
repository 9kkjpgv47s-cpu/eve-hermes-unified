...
describe("run-post-h29-sustainment-loop.mjs", () => {
  it("exposes verify:sustainment-loop npm script (post-H29 terminal sustainment chain)", async () => {
  ...
  it(
    "emits pass and structured checks in post-H29 sustainment loop manifest",
    async () => {
      ...
      const payload = JSON.parse(raw) as {
        pass?: boolean;
        checks?: {
          postH28SustainmentChainPass?: boolean;
          manifestSchemasPostH28LoopEvidencePass?: boolean;
          dispatchContractFixturesEvidencePass?: boolean;
          h29CloseoutGatePass?: boolean;
        };
      };
      expect(payload.pass).toBe(true);
      expect(payload.checks?.postH28SustainmentChainPass).toBe(true);
      expect(payload.checks?.manifestSchemasPostH28LoopEvidencePass).toBe(true);
      expect(payload.checks?.dispatchContractFixturesEvidencePass).toBe(true);
