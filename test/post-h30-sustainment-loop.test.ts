...
describe("run-post-h30-sustainment-loop.mjs", () => {
  it("exposes verify:sustainment-loop npm script (post-H30 terminal chain)", async () => {
  ...
  it(
    "emits pass and structured checks in post-H30 sustainment loop manifest",
    async () => {
      ...
      const payload = JSON.parse(raw) as {
        pass?: boolean;
        checks?: {
          postH29SustainmentChainPass?: boolean;
          manifestSchemasPostH29LoopEvidencePass?: boolean;
          h22CloseoutGatePass?: boolean;
          h30CloseoutGatePass?: boolean;
        };
      };
      expect(payload.pass).toBe(true);
      expect(payload.checks?.postH29SustainmentChainPass).toBe(true);
      expect(payload.checks?.manifestSchemasPostH29LoopEvidencePass).toBe(true);
