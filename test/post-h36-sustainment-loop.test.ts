...
describe("run-post-h36-sustainment-loop.mjs", () => {
  ...
  it(
    "emits pass and structured checks in post-H36 sustainment loop manifest",
    async () => {
      await seedMergeBundleInputs();
      const result = await runCommandWithTimeout(
        ["node", path.join(repoRoot, "scripts/run-post-h36-sustainment-loop.mjs")],
        {
          timeoutMs: 900_000,
      ...
      const payload = JSON.parse(raw) as {
        ...
        checks?: {
          ...
          h36AssuranceBundlePass?: boolean;
          h36CloseoutGatePass?: boolean;
        };
      };
      ...
      expect(payload.checks?.h36AssuranceBundlePass).toBe(true);
      expect(payload.checks?.h36CloseoutGatePass).toBe(true);
    },
    1_200_000,
  );

  it("validate:post-h36-sustainment-manifest passes on latest loop output", async () => {
    const result = await runCommandWithTimeout(
      ["node", path.join(repoRoot, "scripts/validate-post-h36-sustainment-manifest.mjs")],
      { timeoutMs: 15_000 },
    );
    expect(result.code).toBe(0);
  });
