...
describe("run-post-h42-sustainment-loop.mjs", () => {
  it("exposes verify:sustainment-loop npm script (post-H42 terminal chain)", async () => {
  ...
  it(
    "emits pass and structured checks in post-H42 sustainment loop manifest",
    async () => {
      await seedMergeBundleInputs();
      const result = await runCommandWithTimeout(
        ["node", path.join(repoRoot, "scripts/run-post-h42-sustainment-loop.mjs")],
        {
          timeoutMs: 900_000,
      ...
      const payload = JSON.parse(raw) as {
        ...
        checks?: {
          ...
          h42AssuranceBundlePass?: boolean;
          h42CloseoutGatePass?: boolean;
        };
      };
      ...
      expect(payload.checks?.h42AssuranceBundlePass).toBe(true);
      expect(payload.checks?.h42CloseoutGatePass).toBe(true);
    },
    1_200_000,
  );

  it("validate:post-h42-sustainment-manifest passes on latest loop output", async () => {
    const result = await runCommandWithTimeout(
      ["node", path.join(repoRoot, "scripts/validate-post-h42-sustainment-manifest.mjs")],
      { timeoutMs: 15_000 },
    );
    expect(result.code).toBe(0);
  });
