...
describe("release-readiness.mjs", () => {
        ...
        JSON.stringify(
          {
            ...
            checks: {
              ...
              crossSourceOverlapTransitionKeys: ["H2->H3"],
  ...
  it("fails when goal-policy source consistency report is required but missing", async () => {
    await withTempDir(async (dir) => {
      const summaryPath = path.join(dir, "validation-summary-1.json");
      const regressionPath = path.join(dir, "regression-eve-primary-1.json");
      const cutoverPath = path.join(dir, "cutover-readiness-1.json");
      ...
      await writeFile(
        goalPolicyValidationPath,
        JSON.stringify({ pass: true, failures: [], checks: {} }, null, 2),
        "utf8",
      );
      await writeFile(
        commandFile,
      ...
      const result = await runCommandWithTimeout(
        [
          ...
          "--required-command-names",
          "check",
          "--goal-policy-file-validation-report",
          goalPolicyValidationPath,
          "--require-goal-policy-source-consistency-report",
        ],
        { timeoutMs: 10_000 },
      );
      expect(result.code).toBe(2);
  ...
  it("fails when H5 evidence baseline is required but missing", async () => {
    await withTempDir(async (dir) => {
      const summaryPath = path.join(dir, "validation-summary-1.json");
      const regressionPath = path.join(dir, "regression-eve-primary-1.json");
      const cutoverPath = path.join(dir, "cutover-readiness-1.json");
      ...
      const result = await runCommandWithTimeout(
        ...
        {
          timeoutMs: 10_000,
          env: { ...process.env, UNIFIED_RELEASE_READINESS_REQUIRE_H5_BASELINE: "1" },
        },
      );
      expect(result.code).toBe(2);
      const payload = JSON.parse(await readFile(outPath, "utf8")) as {
      ...
      expect(payload.failures).toContain("missing_h5_evidence_baseline_report");
    });
  });
});
