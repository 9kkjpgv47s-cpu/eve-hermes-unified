...
import { createUnifiedMemoryStoreFromEnv, createMemoryStorageKey } from "../src/memory/unified-memory-store.js";
...
describe("file-backed memory durability", () => {
  it("mirrors writes to the shadow file-backed store", async () => {
    ...
    try {
      ...
      const primaryParsed = JSON.parse(await readFile(primary, "utf8")) as unknown[];
      const shadowParsed = JSON.parse(await readFile(shadow, "utf8")) as unknown[];
  ...
  it("scopes storage keys by tenant when tenantId is set", () => {
    const a = createMemoryStorageKey({ lane: "eve", namespace: "n", key: "k", tenantId: "A" });
    const b = createMemoryStorageKey({ lane: "eve", namespace: "n", key: "k", tenantId: "B" });
    const legacy = createMemoryStorageKey({ lane: "eve", namespace: "n", key: "k" });
    expect(a).not.toBe(b);
